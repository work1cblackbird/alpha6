...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СхемаКомпоновкиДанных <> Неопределено Тогда
		...
		// АльфаАвто
		Если Параметры.Свойство("НастройкиВариантаОтчета")
			И ТипЗнч(Параметры.НастройкиВариантаОтчета) = Тип("НастройкиКомпоновкиДанных") Тогда
			Если Параметры.Свойство("ПользовательскиеНастройки")
				...
				ПользовательскиеНастройкиКД = Параметры.ПользовательскиеНастройки;
			Иначе
				ПользовательскиеНастройкиКД = Неопределено;
			КонецЕсли;
			КомпоновщикНастроекКД = Отчет.КомпоновщикНастроек;
			ОтчетыКлиентСервер.ЗагрузитьНастройки(КомпоновщикНастроекКД,
				Параметры.НастройкиВариантаОтчета, ПользовательскиеНастройкиКД);
		КонецЕсли;
		// Конец АльфаАвто
	КонецЕсли;
...
Процедура ПриЗагрузкеВариантаНаСервере(Настройки)
	...
	Если РежимРасшифровки Тогда
		...
		Для Каждого ЭлементПараметров Из Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы Цикл
			...
			ИначеЕсли ИмяПараметра = "КонецПериода" ИЛИ ИмяПараметра = "Период" ИЛИ ИмяПараметра = "ПериодГраница" Тогда
				...
				Если НЕ ТипЗнч(ЭлементПараметров.Значение) = Тип("Граница") Тогда
					КонецПериода = ЭлементПараметров.Значение;
				Иначе
					КонецПериода = ЭлементПараметров.Значение.Значение; 					
				КонецЕсли;	
	...
	// АльфаАвто
	ИначеЕсли Параметры.Свойство("НастройкиВариантаОтчета") И ТипЗнч(Параметры.НастройкиВариантаОтчета) = Тип("НастройкиКомпоновкиДанных") Тогда
		Отчет.КомпоновщикНастроек.ЗагрузитьНастройки(Параметры.НастройкиВариантаОтчета);
	// Конец АльфаАвто
	КонецЕсли;
	
	// Загрузим фиксированные отборы
	ПериодОтчетаУстановлен = Ложь;
	...
	Если НЕ РежимРасшифровки И НЕ ПериодОтчетаУстановлен Тогда
		ПолучитьПериодВСоответствииСВариантомОтчета(Настройки);
	КонецЕсли;	
...
Процедура ВыбратьВариантНаСервере(ИмяВарианта)
	...
	Если ТиповойВариантНастроек = Неопределено Тогда
		...
		КонецЕсли;
	Иначе  
		Настройки = ТиповойВариантНастроек.Настройки;
...
//
&НаКлиенте
Процедура ОткрытьНастройкиОтчета(Команда) Экспорт
...
Процедура ОтборВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ОтборЗаголовок" Тогда
		...
		ОписаниеОповещенияВыбораПоля = Новый ОписаниеОповещения(
			"Подключаемый_ОповещениеВыбораПоля",
			ЭтотОбъект,
...
Процедура Подключаемый_ОбработчикОжиданияОтборПриАктивизацииСтроки()
	...
	НастроитьСвязиПараметровВыбора(ТекущиеДанные.ЛевоеЗначение);
...
Процедура ОтборПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ОписаниеОповещенияВыбораПоля = Новый ОписаниеОповещения(
		"Подключаемый_ОтборПередНачаломДобавленияЗавершение",
		ЭтотОбъект
	);
	ОткрытьФорму(
		"ОбщаяФорма.ФормаВыбораПолейОтчета",
...
Процедура ОтборПередУдалением(Элемент, Отказ)
	...
	Для Каждого Строка Из ЗначенияРеквизитовОтборов Цикл
		Если Строка.Ключ = ТекущиеДанные.ЛевоеЗначение Тогда
			ЭтотОбъект[Строка.Значение] = Неопределено;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОтборИспользованиеПриИзменении(Элемент)
	...
	ОбновитьСвязиПараметровВыбора(
		ТекущиеДанные.ЛевоеЗначение,
		ТекущиеДанные.ВидСравнения,
		ТекущиеДанные.ПравоеЗначение,
...
Процедура СтрокиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ОписаниеОповещенияВыбораПоля = Новый ОписаниеОповещения(
		"Подключаемый_СтрокиПередНачаломДобавленияЗавершение",
		ЭтотОбъект
	);
	ОткрытьФорму(
		"ОбщаяФорма.ФормаВыбораПолейОтчета",
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Значение" в контексте сервера.
//
&НаСервере
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Значение".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
...
Процедура КолонкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ОписаниеОповещенияВыбораПоля = Новый ОписаниеОповещения(
		"Подключаемый_КолонкиПередНачаломДобавленияЗавершение",
		ЭтотОбъект
	);
	ОткрытьФорму(
		"ОбщаяФорма.ФормаВыбораПолейОтчета",
...
Процедура СерииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("СхемаКомпоновки", СхемаКомпоновки);
	СтруктураПараметров.Вставить("КомпоновщикНастроек", Отчет.КомпоновщикНастроек);
...
Функция СформироватьОтчетВФоне(ОбновитьНастройки = Истина)
	...
	Если КлючОбъекта = "Отчет.ПечатьРеестра" Тогда
		Попытка
			Отчет.КомпоновщикНастроек.Настройки.Структура[0].Строки[0].Порядок.Элементы.Очистить();
			Для Каждого ЭлементПорядка Из Отчет.КомпоновщикНастроек.Настройки.Порядок.Элементы Цикл
				ЭлементПорядкаОтчет = Отчет.КомпоновщикНастроек.Настройки.Структура[0].Строки[0].Порядок.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных")); 
				ЭлементПорядкаОтчет.Поле = ЭлементПорядка.Поле;
				ЭлементПорядкаОтчет.ТипУпорядочивания = ЭлементПорядка.ТипУпорядочивания;
				ЭлементПорядкаОтчет.Использование = ЭлементПорядка.Использование;
			КонецЦикла;
		Исключение
		КонецПопытки;
	КонецЕсли;	
...
Процедура ИзменитьГруппировкуКолонки()
	...
	Если ТекущиеДанные.ПоляГруппировки.Количество()>0 Тогда
		СтрокаГруппировки = ТекущиеДанные.ПоляГруппировки[0];
		ПараметрыВыбора.Вставить("ТекущееПоле",     СтрокаГруппировки.Поле);
		ПараметрыВыбора.Вставить(
			"ТипГруппировки",
			ПолучитьПолноеИмяПредопределенногоЗначения(СтрокаГруппировки.ТипГруппировки)
		);
	КонецЕсли;
...
Процедура ОбновитьБыстрыеНастройки(Вариант = Неопределено)
	...
	Для Каждого ЭлементОтбора Из Настройки.Отбор.ДоступныеПоляОтбора.Элементы Цикл
		
		Если ЭлементОтбора.Поле = Новый ПолеКомпоновкиДанных("ПользовательскиеПоля") Тогда
			
			Для Каждого ЭлементОтбораПользовательскогоПоля Из ЭлементОтбора.Элементы Цикл
				
				ПродолжитьЗаполнениеПолейСвязиПараметровВыбора(
					ЭлементОтбораПользовательскогоПоля,
					Счетчик,
					НовыеРеквизиты,
					Настройки);
		...
		Иначе
			...
			ПродолжитьЗаполнениеПолейСвязиПараметровВыбора(ЭлементОтбора, Счетчик, НовыеРеквизиты, Настройки);
...
&НаСервере
Процедура ПродолжитьЗаполнениеПолейСвязиПараметровВыбора(ЭлементОтбора, Счетчик, НовыеРеквизиты, Настройки)
	
	СвязиПараметровВыбора = ЭлементОтбора.ПолучитьСвязиПараметровВыбора();
	
	Для Каждого Связь Из СвязиПараметровВыбора Цикл
		ДоступноеПоле = Настройки.ДоступныеПоляОтбора.НайтиПоле(Связь.Поле);
		Если ДоступноеПоле = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	
		ПараметрыОтбора = Новый Структура("Ключ", Связь.Поле);
		НайденныеСтроки = ЗначенияРеквизитовОтборов.НайтиСтроки(ПараметрыОтбора);
		Если НайденныеСтроки.Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		
		НовыйРеквизит = Новый РеквизитФормы(
			Строка(СтрЗаменить(Связь.Имя, ".", "") + "_" + СтрЗаменить(Новый УникальныйИдентификатор, "-", "")),
			ДоступноеПоле.ТипЗначения,
			,
			СтрЗаменить(Связь.Имя, ".", "")
		);
		Счетчик = Счетчик + 1;
		НовыеРеквизиты.Добавить(НовыйРеквизит);
		
		НоваяСтрока = ЗначенияРеквизитовОтборов.Добавить();
		НоваяСтрока.Ключ = Связь.Поле;
		НоваяСтрока.Значение =  НовыйРеквизит.Имя;
		НоваяСтрока.ПолеОтбора = ЭлементОтбора.Поле;
		НоваяСтрока.Очищать = Связь.ИзменениеЗначения = РежимИзмененияСвязанногоЗначения.Очищать;
		Строки = СтрРазделить(Связь.Имя, ".");
		Если ЗначениеЗаполнено(Строки) Тогда
			Если Строки.Количество() > 1 Тогда
				НоваяСтрока.Имя = Строки[1];
			Иначе
				НоваяСтрока.Имя = Строки[0];
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры // ПродолжитьЗаполнениеПолейСвязиПараметровВыбора()
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(НастройкиСохранения)
	...
	Если ПолучитьЗначениеПараметраСтруктуры(НастройкиСохранения, "ПереводитьИменаФайловВТранслит", Ложь) Тогда
...
&НаСервере
Процедура ПолучитьПериодВСоответствииСВариантомОтчета(Настройки) 
...
&НаСервере
Процедура НастроитьСвязиПараметровВыбора(ЛевоеЗначение)
	
	МассивСвязей = Новый Массив();
	
	ДоступноеПоле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.НайтиПоле(ЛевоеЗначение);
	
	СвязиПараметровВыбора = ДоступноеПоле.ПолучитьСвязиПараметровВыбора();
	...
	Для Каждого Связь Из СвязиПараметровВыбора Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			Продолжить;
		КонецЕсли;
		ЭтотОбъект[НайденныеСтроки[0].Значение] = Неопределено;
		Для Каждого ЭлементОтбора Из Отбор.ПолучитьЭлементы() Цикл
			Если Связь.Поле = ЭлементОтбора.ЛевоеЗначение
				И ЭлементОтбора.Использование
				И ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
				ЭтотОбъект[НайденныеСтроки[0].Значение] = ЭлементОтбора.ПравоеЗначение;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		НоваяСвязь = Новый СвязьПараметраВыбора(Связь.Имя, НайденныеСтроки[0].Значение);
		МассивСвязей.Добавить(НоваяСвязь);
	КонецЦикла;
	
	Элементы.ОтборПравоеЗначение.СвязиПараметровВыбора = Новый ФиксированныйМассив(МассивСвязей);
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСвязиПараметровВыбора(ЛевоеЗначение, ВидСравнения, ПравоеЗначение, Использование)
	
	ДоступноеПоле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.НайтиПоле(ЛевоеЗначение);
	
	Для Каждого Строка Из ЗначенияРеквизитовОтборов Цикл
		
		Если Строка.Ключ = ДоступноеПоле.Поле Тогда
			Если Использование И ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
				ЭтотОбъект[Строка.Значение] = ПравоеЗначение;
			Иначе
				ЭтотОбъект[Строка.Значение] = Неопределено;
			КонецЕсли;
			
			Если Использование И Строка.Очищать И ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
				ПерезаполнитьОтборыПоСвязямПараметровВыбора(Отбор, Строка);
			КонецЕсли;
		КонецЕсли;
...
&НаСервере
Процедура ПерезаполнитьОтборыПоСвязямПараметровВыбора(ДеревоОтбора, ЗначениеРеквизитовОтборов)
	
	Для Каждого Строка Из ДеревоОтбора.ПолучитьЭлементы() Цикл
		
		Если Строка.Папка Тогда
			ПерезаполнитьОтборыПоСвязямПараметровВыбора(Строка.Строки, ЗначениеРеквизитовОтборов.ПолеОтбора);
		КонецЕсли;
		
		Если Строка.ЛевоеЗначение = ЗначениеРеквизитовОтборов.ПолеОтбора Тогда
			
			Если Не ЗначениеЗаполнено(Строка.ПравоеЗначение) Тогда
				Продолжить;
			КонецЕсли;
		
			Если ЗначениеЗаполнено(ЗначениеРеквизитовОтборов.Имя) Тогда
				Попытка
					ПроверяемоеЗначение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
						Строка.ПравоеЗначение,
						ЗначениеРеквизитовОтборов.Имя
					);
				Исключение
					ПроверяемоеЗначение = Неопределено;
				КонецПопытки;
				
				Если ПроверяемоеЗначение <> ЭтотОбъект[ЗначениеРеквизитовОтборов.Значение] Тогда
					Строка.ПравоеЗначение = Неопределено;
