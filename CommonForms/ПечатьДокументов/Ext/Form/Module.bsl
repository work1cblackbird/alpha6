...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	ПравоСохранения = ПраваИНастройкиПользователя.Значение("СохранениеСтандартныхПараметровПечати")
		И ПравоДоступа("Вывод", Метаданные);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"СохранитьТекущиеПараметрыКакСтандартные",
		"Видимость",
		ПравоСохранения
	);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"СохранитьТекущиеПараметрыКакСтандартныеВсеДействия",
		"Видимость",
		ПравоСохранения
	);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"УстановитьСтандартныеПараметры",
		"Доступность",
		ПравоДоступа("Вывод", Метаданные)
	);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"УстановитьСтандартныеПараметрыВсеДействия",
		"Доступность",
		ПравоДоступа("Вывод", Метаданные)
	);
	
	// ПростаяЭлектроннаяПодпись
	ПараметрыПечатьДокументов = ДокументыУчетаПЭП.НовыеПараметрыПриСозданииНаСервере_ПечатьДокументов();
	ПараметрыПечатьДокументов.МестоРазмещения = Элементы.КоманднаяПанельЛеваяЧасть;
	ДокументыУчетаПЭП.ПриСозданииНаСервере_ПечатьДокументов(ЭтотОбъект, ПараметрыПечатьДокументов);
...
Процедура Подключаемый_ВыполнитьКомандуПЭП(Команда)
	...
	Результат = ВыполнитьКомандуПЭПНаСервере();
	ОткрытьФорму(
		"Документ.ЗаявкаНаПодпись.Форма.ФормаДокумента",
		Результат,
		ЭтотОбъект,
		ЭтотОбъект
	);
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(ПереданныеНастройки)
	...
	// АльфаАвто +
	ПечатьКомплекта = (НастройкиПечатныхФорм.Количество() > 1);
	// АльфаАвто -
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		Для Каждого СоответствиеОбъектаПечатнойФорме Из ПечатныеФормыПоОбъектам Цикл
			...
			Для Каждого ВыбранныйФормат Из ВыбранныеФорматыСохранения Цикл
				...
				// АльфаАвто
				Если ПереданныеНастройки.Свойство("ИмяФайла")
					И ЗначениеЗаполнено(ПереданныеНастройки.ИмяФайла)Тогда
					ИмяФайла = УправлениеПечатью.ИмяФайлаПечатнойФормыОбъекта(ОбъектПечати, ЗаданныеИменаПечатныхФорм,
						ПереданныеНастройки.ИмяФайла + ?(ПечатьКомплекта, " (" + НастройкаПечатнойФормы.Название + ")", ""));	
				КонецЕсли;
				// Конец АльфаАвто
...
Функция ЕстьРазрешенноеРедактирование()
	...
	// АльфаАвто
	РедактированиеРазрешено = Не ПравоПользователя("ЗащитаПечатныхФорм");
	
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Элементы[НастройкаПечатнойФормы.ИмяРеквизита].Защита = Не РедактированиеРазрешено;
	КонецЦикла;
...
Процедура УстановитьСтандартныеПараметрыНаСервере()
	...
	УправлениеПечатьюПлатформа.
		УстановитьСтандартныеПараметрыПечати(ТекущаяПечатнаяФорма.ИмяПараметровПечати, ТекущаяПечатнаяФорма);
...
Функция СохранитьТекущиеПараметрыКакСтандартныеНаСервере()
	...
	УправлениеПечатьюПлатформа.
		СохранитьТекущиеПараметрыКакСтандартные(ТекущаяПечатнаяФорма.ИмяПараметровПечати,ТекущаяПечатнаяФорма);
	
КонецФункции //СохранитьТекущиеПараметрыКакСтандартныеНаСервере()

// ПростаяЭлектроннаяПодпись
&НаСервере
Функция ВыполнитьКомандуПЭПНаСервере()
	
	Возврат ДокументыУчетаПЭП.ПодготовитьДанныеДляЗаявкиНаПодписьПоПечатнойФорме(ЭтотОбъект);
