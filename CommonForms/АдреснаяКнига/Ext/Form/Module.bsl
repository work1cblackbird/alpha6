...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	// Внесены правки, чтобы данную форму открыть из документа рассылка
	...
	Если ТипКИ = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
		Элементы.ПоказыватьТолькоКонтактыСАдресами.Заголовок = НСтр("ru = 'Только контакты с телефонами'");
		Элементы.ПоказыватьТолькоКонтактыСАдресами.РасширеннаяПодсказка.Заголовок = 
			НСтр("ru = '    Будет применено только к тем спискам, в которых есть колонка ""Телефон""'");
		Элементы.ПолучателиПисьма.Заголовок = НСтр("ru = 'Получатели'");
	КонецЕсли;
	// Конец АльфаАвто
	
	Взаимодействия.ДобавитьСтраницыФормыПодбораКонтактов(ЭтотОбъект, ТипКИ); // АльфаАвто
...
Процедура Подключаемый_СписокСправочникаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// АльфаАвто
	Результат = ВзаимодействияАльфаАвтоВызовСервера.НаименованиеИАдресаЭлектроннойПочтыКонтакта(
		ВыбраннаяСтрока,
		ТипКИ);
	...
	Если НЕ ПустаяСтрока(СписокАдресов) Тогда
...
Процедура НайденныеКонтактыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// АльфаАвто
	// Изменено для того чтобы получать различную контактную информация, в зависимости от типа.
	Результат = ВзаимодействияАльфаАвтоВызовСервера.НаименованиеИАдресаЭлектроннойПочтыКонтакта(
...
Процедура ВключитьВСписокПолучателей(Команда)
	...
	Если Не ЭлементСправочника = Неопределено Тогда
		ПоказатьЗначение(, ЭлементСправочника);
	КонецЕсли;
	// Конец АльфаАвто
КонецПроцедуры
...
Процедура ЗаполнитьТаблицуПолучателей()
	...
	Если табПолучатели.Количество() > 0 Тогда
		...
		ТаблицаАдресов = ВзаимодействияАльфаАвто.АдресаЭлектроннойПочтыКонтактов(
			табПолучатели.ВыгрузитьКолонку("Контакт"), ТипКИ);
		// Конец АльфаАвто
			Запрос = Новый Запрос;
			Запрос.Текст = "
			|ВЫБРАТЬ
...
&НаСервере
Процедура ПеренестиВыделенныеСтроки(Знач ВыделенныеСтроки, ЭлементСправочника) // АльфаАвто
	
	// АльфаАвто
	// Изменено для того чтобы получать КИ в зависимости от переданного типа.
	Результат = ВзаимодействияАльфаАвто.АдресаЭлектроннойПочтыКонтактов(ВыделенныеСтроки, ТипКИ, ГруппаПоУмолчанию);
	// Конец АльфаАвто
	Если Результат <> Неопределено Тогда
		УдалитьПустогоПолучателя(ПолучателиПисьма);
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Результат, ПолучателиПисьма);
		// АльфаАвто
		Если Результат.Количество() = 0 И ВыделенныеСтроки.Количество() = 1 Тогда
			ЭлементСправочника = ВыделенныеСтроки[0].Ссылка;
		КонецЕсли;
		// Конец АльфаАвто
	КонецЕсли;
