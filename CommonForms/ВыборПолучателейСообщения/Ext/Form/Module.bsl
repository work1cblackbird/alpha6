...
Функция СформироватьПочтовыйАдресПолучателя()
	...
	Для Каждого Получатель Из ПолучателиСообщения Цикл
		Если Получатель.Выбран Тогда
			ПочтовыйАдресПолучателя = ПочтовыйАдресПолучателя + Получатель.Представление 
					+ " <" + СокрЛП(Получатель.Адрес) + ">; ";
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьТаблицуПолучателейИзМассиваСтруктур(ПараметрыПолучателейСообщения)
	...
	Для Каждого ПараметрыПолучателя Из ПараметрыПолучателейСообщения Цикл
		...
		Если ПараметрыПолучателя.Свойство("ВидПочтовогоАдреса") 
				И НЕ ПустаяСтрока(ПараметрыПолучателя.ВидПочтовогоАдреса) Тогда
			Получатель.ПредставлениеАдреса = Получатель.ПредставлениеАдреса 
				+ " (" + ПараметрыПолучателя.ВидПочтовогоАдреса + ")";
		КонецЕсли;
	КонецЦикла;
