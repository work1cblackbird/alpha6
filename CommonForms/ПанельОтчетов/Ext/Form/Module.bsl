...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	ВариантыОтчетовАльфаАвто.ДополнитьЭлементамиПанельОтчетов(ЭтотОбъект);
	// Конец АльфаАвто
...
Функция ЗаполнитьПанельОтчетовВФоне()
	...
	// АльфаАвто
	ПараметрыПоиска.Вставить("ОтборПоТипуДоступа", Истина);
	// Конец АльфаАвто
	...
	Если ВариантыОтчетов.ПредставленияЗаполнены() = "Заполнены" Тогда
		...
		// АльфаАвто
		ВариантыОтчетовАльфаАвто.НайтиВариантыОтчетовДляВывода(ПараметрыПоиска, АдресРезультата);
		// Конец АльфаАвто
	Иначе
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
		// АльфаАвто
		Результат = ДлительныеОперации.ВыполнитьВФоне("ВариантыОтчетовАльфаАвто.НайтиВариантыОтчетовДляВывода", ПараметрыПоиска, ПараметрыВыполнения);
		// Конец АльфаАвто
		Если Результат.Статус <> "Выполнено" Тогда
			Возврат Результат;
		КонецЕсли;
...
Функция ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ВГруппу, УровеньВложенности = 0)
	...
	Иначе
		...
		НомерГруппы = 0;
		ИмяОтступа = "ГруппаОтступаВарианта_%1";
		Пока Элементы.Найти(СтрШаблон(ИмяОтступа, Формат(НомерГруппы, "ЧГ=0"))) <> Неопределено Цикл
			НомерГруппы = НомерГруппы + 1;
		КонецЦикла;
		
		ГруппаОтступаИмя   = СтрШаблон(ИмяОтступа, Формат(НомерГруппы, "ЧГ=0"));
		КартинкаОтступаИмя = "КартинкаОтступаВарианта_" + Формат(НомерГруппы, "ЧГ=0");
...
// АльфаАвто
&НаКлиенте
Процедура Подключаемый_ТекущийРазделСсылкаПриИзменении(Элемент)
