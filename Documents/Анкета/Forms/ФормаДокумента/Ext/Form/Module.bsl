...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьКомандуОтказОтАнкетирования();
...
&НаСервере
Функция ОкончаниеРедактированиеФормыЗаполнения(РежимЗаписи, ДополнительныеПараметры = Неопределено) // АльфаАвто Добавил параметр "ДополнительныеПараметры"
	
	ПреобразоватьРезультатыЗаполненияРазделаВТабличнуюЧасть();
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		// АльфаАвто
		Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
			ПроверятьЗаполнение = Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ДополнительныеПараметры,
				"ОтказОтАнкетирования",
				Ложь
			);
		Иначе
		...
		КонецЕсли;
		// Конец АльфаАвто
		Если ПроверятьЗаполнение И ПроверитьЗаполнениеАнкеты() Тогда // АльфаАвто
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВопросОПринятииАнкетыПослеЗавершения(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура")
		И ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "ОтказОтАнкетирования", Ложь) Тогда
		Объект.ОтказОтАнкетирования = Истина;
	КонецЕсли;
	// Конец АльфаАвто
	
	Отказ = ОкончаниеРедактированиеФормыЗаполнения(РежимЗаписиДокумента.Проведение, ДополнительныеПараметры); // АльфаАвто
...
Процедура ДействияВЗависимостиОтВидаФормы()
	...
	Если Параметры.Свойство("ТолькоФормаЗаполнения") Тогда
		...
		ЭтоИнтервью = Объект.РежимАнкетирования = ПредопределенноеЗначение("Перечисление.РежимыАнкетирования.Интервью");
		// АльфаАвто Использовано УстановитьСвойствоЭлементаФормы, т.к. возможно отсутствие прав на реквизиты.
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаОсновныеРеквизиты", 				"Видимость", ЭтоИнтервью);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Дата", 									"Доступность", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаКоманднаяПанельПровестиИЗакрыть", 	"Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаКоманднаяПанельЗаписать", 			"Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Провести", 								"Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОтменаПроведения", 				"Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаПеречитать", 						"Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПоказатьВСпискеАнкета", 				"Видимость", Ложь);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "РежимАнкетирования", 	"ТолькоПросмотр", ЭтоИнтервью);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Опрос", 				"ТолькоПросмотр", ЭтоИнтервью);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ШаблонАнкеты", 			"ТолькоПросмотр", ЭтоИнтервью);
		// Конец АльфаАвто
		Если Объект.Ссылка.Пустая() Тогда
			Объект.Дата = ТекущаяДатаСеанса();
		КонецЕсли;
		...
		Если Параметры.ТолькоПросмотр = Истина Тогда
			// АльфаАвто Использовано УстановитьСвойствоЭлементаФормы, т.к. возможно отсутствие прав на реквизиты.
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаТелоАнкеты", 					"ТолькоПросмотр", Истина);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗаполненияПровестиЗакрыть", 	"Видимость", Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗаполненияЗаписать", 			"Видимость", Ложь);
			// Конец АльфаАвто
		Иначе
			// АльфаАвто Использовано УстановитьСвойствоЭлементаФормы, т.к. возможно отсутствие прав на реквизиты.
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗаполненияЗаписать", "Видимость", ВозможностьПредварительногоСохранения);
			// Конец АльфаАвто
		КонецЕсли;
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗаполненияЗаписать", 			"Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаТелоАнкеты", 					"ТолькоПросмотр", Истина); 
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаОсновныеРеквизиты", 			"ТолькоПросмотр", Истина);
		// Конец АльфаАвто
	КонецЕсли;
...
// АльфаАвто
&НаСервере
Процедура ДобавитьКомандуОтказОтАнкетирования()
	
	НоваяКоманда = Команды.Добавить("ОтказОтАнкетирования");
	НоваяКоманда.Действие = "КомандаОтказОтАнкетирования";
	НоваяКоманда.Заголовок = НСтр("ru = 'Отказ от анкетирования'");
	НоваяКоманда.Подсказка = НСтр("ru = 'Отказ от анкетирования'");
	
	НовыйЭлемент = Элементы.Добавить("КнопкаОтказОтАнкетирования", Тип("КнопкаФормы"), Элементы.КоманднаяПанельШапка);
	НовыйЭлемент.ИмяКоманды = "ОтказОтАнкетирования";
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"КнопкаОтказОтАнкетирования",
		"Доступность",
		Не Объект.ОтказОтАнкетирования
	);
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаОтказОтАнкетирования(Команда)
	
	Модифицированность = Ложь;
	ДополнительныеПараметры = Новый Структура("ОтказОтАнкетирования", Истина);
	ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения(
		"ВопросОПринятииАнкетыПослеЗавершения",
		ЭтотОбъект,
		ДополнительныеПараметры
	);
	ПоказатьВопрос(ОбработчикОповещенияОЗакрытии,
		НСтр("ru = 'Ваша анкета будет принята с отказом.
			|Дальнейшее заполнение анкеты будет невозможно
			|Продолжить?'")
