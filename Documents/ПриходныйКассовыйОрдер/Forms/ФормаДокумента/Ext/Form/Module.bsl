...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	РаботаСФормойКлиент.ЗамерВремениЗапись("ПриходныйКассовыйОрдер", ПараметрыЗаписи.РежимЗаписи,
...
Процедура ДляПробитияНаФРПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Если Объект.ДляПробитияНаФР Тогда
		Если НЕ ЗначениеЗаполнено(Объект.ТипРасчета) Тогда
			Объект.ТипРасчета = Документы.ПриходныйКассовыйОрдер.ТипРасчетаПоДокументу(Объект);
		КонецЕсли;
	КонецЕсли;
...
Процедура РасшифровкаПлатежаДокументРасчетовБезРазбиенияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Иначе
		ТекущиеДанные = Объект.Состав.НайтиПоИдентификатору(Элементы.Состав.ТекущаяСтрока);
	КонецЕсли;
	Документы.ПриходныйКассовыйОрдер.СоставДокументРасчетовПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	ПерезаполнитьТоварыНаСервере();
...
Процедура РасшифровкаПлатежаЗаказБезРазбиенияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Иначе
		ТекущиеДанные = Объект.Состав.НайтиПоИдентификатору(Элементы.Состав.ТекущаяСтрока);
	КонецЕсли;
	Документы.ПриходныйКассовыйОрдер.СоставЗаказПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	ПерезаполнитьТоварыНаСервере();
...
&НаСервере
Процедура СтатьяДДСПриИзмененииНаСервере(ПараметрыДействия)
	Документы.ПриходныйКассовыйОрдер.СтатьяДДСПриИзменении(Объект, ПараметрыДействия);
	УправлениеДиалогомНаСервере();
КонецПроцедуры
...
&НаКлиенте
Процедура СоставПлатежаСпособЗачетаОплатыПриИзменении(Элемент)
...
Процедура СоставСуммаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Состав.НайтиПоИдентификатору(Элементы.Состав.ТекущаяСтрока);
	Документы.ПриходныйКассовыйОрдер.СоставСуммаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура СоставСтавкаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Состав.НайтиПоИдентификатору(Элементы.Состав.ТекущаяСтрока);
	Документы.ПриходныйКассовыйОрдер.СоставСтавкаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ПодборОплаты(Команда)
	...
	ПараметрыПодбора.Вставить("РасчетыСПокупателем", Истина);
...
Процедура ПечатьЧекаЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ ТипЗнч(РезультатВыполнения) = Тип("Булево") Тогда
		...
		Иначе
			...
			РезультатВыполнения.Вставить("Кассир", Кассир);
			РезультатВыполнения.Вставить("КассирИНН", ИННКассира);
...
Процедура НастроитьПараметрыВыбораЭлементовФормы()
	...
	// Ограничим выбор вида операции чека только теми, по которым возможен приход денежных средств.
	...
	ВидДвиженияПараметры.Добавить(Перечисления.ВидыДвижений.Приход);
...
Процедура УправлениеДиалогомНаСервере()
	...
	ЭтоПеремещение = Объект.ВидОперации = Перечисления.ВидыОперацийДвиженияДенежныхСредств.ПолучениеНаличныхВБанке;
