...
Функция ТипРасчетаПоДокументу(Объект, УчитыватьСтатьюДДС = Истина) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование) И НЕ ЗначениеЗаполнено(Объект.Сделка) Тогда
		...
		СтатьиВозврата = Новый Массив;
		СтатьиВозврата.Добавить(Справочники.СтатьиДДС.ВозвратИзПодОтчета);
		СтатьиВозврата.Добавить(Справочники.СтатьиДДС.ВозвратОтПоставщика);
		
		Если УчитыватьСтатьюДДС И СтатьиВозврата.Найти(Объект.СтатьяДДС) <> Неопределено Тогда
	...
	Если ДокументыВозвратаПрихода.Найти(ТипЗнчСделки) = Неопределено
		И ДокументыВозвратаПрихода.Найти(ТипЗнчОснования) = Неопределено Тогда
		Возврат Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств;
	Иначе
		Возврат Перечисления.ТипыРасчетаДенежнымиСредствами.ВозвратРасходаДенежныхСредств;
	КонецЕсли;
...
Функция ПроверитьВидОперацииПеремещение(Операция) Экспорт
	...
	Если Операция = Неопределено Тогда
		Возврат Ложь;
	Иначе
		Возврат Операция = Перечисления.ВидыОперацийДвиженияДенежныхСредств.ПолучениеНаличныхВБанке;
...
Процедура ИнициализироватьДанныеДокументаДляПроведения(Объект, ДополнительныеПараметры) Экспорт
	...
	|	ПриходныйКассовыйОрдер.Сделка КАК Сделка,
	|	ПриходныйКассовыйОрдер.Контрагент КАК Контрагент,
	|	ПриходныйКассовыйОрдер.ДоговорВзаиморасчетов КАК ДоговорВзаиморасчетов,
	|	ПриходныйКассовыйОрдер.ДоговорВзаиморасчетов.ВалютаВзаиморасчетов КАК ВалютаДоговора,
	|	ПриходныйКассовыйОрдер.СуммаДокумента КАК СуммаДокумента,
	|	ПриходныйКассовыйОрдер.КурсВалютыВзаиморасчетов КАК КурсВалютыВзаиморасчетов,
	|	ПриходныйКассовыйОрдер.КурсВалютыУпр КАК КурсВалютыУпр,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПриходныйКассовыйОрдерСостав.Ссылка КАК Ссылка,
	|	ПриходныйКассовыйОрдерСостав.ДокументРасчетов КАК ДокументРасчетов,
	|	ПриходныйКассовыйОрдерСостав.Заказ КАК Заказ,
	|	ПриходныйКассовыйОрдерСостав.Сумма КАК Сумма,
	|	ПриходныйКассовыйОрдерСостав.СтавкаНДС КАК СтавкаНДС,
	|	ПриходныйКассовыйОрдерСостав.СуммаНДС КАК СуммаНДС,
	|	ПриходныйКассовыйОрдерСостав.СпособЗачетаОплаты КАК СпособЗачетаОплаты
	|ПОМЕСТИТЬ РасшифровкаПлатежа
	|ИЗ
	|	Документ.ПриходныйКассовыйОрдер.Состав КАК ПриходныйКассовыйОрдерСостав
	|ГДЕ
	|	ПриходныйКассовыйОрдерСостав.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура КонтрагентПриИзменении(Объект, ПараметрыДействия=Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		...
		Объект.ПринятоОТ = СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Контрагент, "НаименованиеПолное", Истина));
...
Процедура СделкаПриИзменении(Объект, ПараметрыДействия=Неопределено) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Сделка) Тогда
		Отбор = Новый Структура();
		Отбор.Вставить("Контрагент"            , Объект.Контрагент);
		Отбор.Вставить("ДоговорВзаиморасчетов" , Объект.ДоговорВзаиморасчетов);
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	ОбъектыВводимыеНаОсновании.Добавить(Документы.ПеремещениеДенежныхСредств);
...
//
// Параметры:
//  ДокументОбъект - ДокументОбъект.ПриходныйКассовыйОрдер - документ, для которого выполняется действие.
...
Функция ВозможенВводСчетФактурыВыданной(ДокументОбъект, ДополнительныеПарметры = Неопределено) Экспорт
	...
	ДанныеДокументОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОбъект.Ссылка, "Организация, СтавкаНДС");
