///////////////////////////////////////////////////////////////////////////////
// Модуль основной формы документа "Рабочий лист отдела страхования"
//
///////////////////////////////////////////////////////////////////////////////
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЕстьРеквизит(ЭтотОбъект,"РеспондентОпроса") Тогда
		ЭтотОбъект.РеспондентОпроса = Объект.Страхователь;
	КонецЕсли; 
	...
	Элементы.ВариантыСтрахованияВвестиСтраховойПолис.Доступность = ПравоДоступа("Добавление", Метаданные.Документы.СтраховойПолис);
	...
	Если Объект.Страхователь = Справочники.Контрагенты.ПустаяСсылка() ИЛИ Объект.Страхователь = Неопределено Тогда
		Объект.Страхователь = "";
...
Процедура ПриОткрытии(Отказ)
	...
	ЗащищенныеФункцииКлиент.НастроитьЭлементФормыТабличнойЧасти(ЭтаФорма, "ВариантыСтрахования");
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	РаботаСФормойКлиент.ЗамерВремениЗапись("РабочийЛистОтделаСтрахования", ПараметрыЗаписи.РежимЗаписи, Ложь);
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.РабочийЛистОтделаСтрахования.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.РабочийЛистОтделаСтрахования.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура СтраховательПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	...
	Документы.РабочийЛистОтделаСтрахования.СтраховательПриИзменении(Объект, ПараметрыДействия); 
...
Процедура СтраховательПриИзменении(Элемент)
	...
	ОбработатьИзменениеСтрахователя();
	
КонецПроцедуры 

&НаКлиенте
Процедура СтраховательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Массив") И ВыбранноеЗначение.Количество() Тогда
		СтандартнаяОбработка = Ложь;
		Объект.Страхователь = ВыбранноеЗначение[0].КонтактноеЛицо;
		АдресИзДанныхВыбора = "";
...
&НаКлиенте
Процедура СтраховательАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СтраховательОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
...
Процедура ПослеВыбораИзМенюТелефон(ВыбранныйЭлемент, Параметры) Экспорт
	...
	Если ВыбранныйЭлемент <> Неопределено Тогда
		...
		Иначе
			Если ТипЗнч(Объект.Страхователь) = Тип("СправочникСсылка.Контрагенты") Тогда
				ВзаимодействияАльфаАвтоКлиент.ПозвонитьКонтакту(Объект.Страхователь, ПредставлениеТелефона, Объект.Ссылка);
			Иначе
...
Процедура ПроверитьНеобходимостьОбновленияКИ()
	...
	Если ТипЗнч(Объект.Страхователь) = Тип("Строка")
		ИЛИ Объект.Страхователь = Справочники.Контрагенты.ПустаяСсылка() Тогда
		Элементы.ГруппаРежимОбновленияКонтактнойИнформации.Доступность = Ложь;
		Возврат;
	КонецЕсли;
...
#Область ОбработчикиСобытийПолейТаблицыФормыВариантыСтрахования

&НаСервере
Процедура ВариантыСтрахованияВидСтрахованияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияВидСтрахованияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияВидСтрахованияПриИзменении(Элемент)
	
	ВариантыСтрахованияВидСтрахованияПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияПрограммаСтрахованияПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияПрограммаСтрахованияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияПрограммаСтрахованияПриИзменении(Элемент)
	
	ВариантыСтрахованияПрограммаСтрахованияПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияСтраховщикПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияСтраховщикПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияСтраховщикПриИзменении(Элемент)
	
	ВариантыСтрахованияСтраховщикПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияТарифПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияТарифПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияТарифПриИзменении(Элемент)
	
	ВариантыСтрахованияТарифПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияСтраховаяСуммаПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияСтраховаяСуммаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияСтраховаяСуммаПриИзменении(Элемент)
	
	ВариантыСтрахованияСтраховаяСуммаПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияСуммаБруттоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияСуммаБруттоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияСуммаБруттоПриИзменении(Элемент)
	
	ВариантыСтрахованияСуммаБруттоПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияСуммаНеттоПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияСуммаНеттоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияСуммаНеттоПриИзменении(Элемент)
	
	ВариантыСтрахованияСуммаНеттоПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияПроцентСкидкиСтрокиПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияПроцентСкидкиСтрокиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияПроцентСкидкиСтрокиПриИзменении(Элемент)
	
	ВариантыСтрахованияПроцентСкидкиСтрокиПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияСуммаСкидкиСтрокиПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияСуммаСкидкиСтрокиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияСуммаСкидкиСтрокиПриИзменении(Элемент)
	
	ВариантыСтрахованияСуммаСкидкиСтрокиПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияПроцентКомиссииПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияПроцентКомиссииПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияПроцентКомиссииПриИзменении(Элемент)
	
	ВариантыСтрахованияПроцентКомиссииПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияСуммаКомиссииПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияСуммаКомиссииПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияСуммаКомиссииПриИзменении(Элемент)
	
	ВариантыСтрахованияСуммаКомиссииПриИзмененииНаСервере();
	
КонецПроцедуры 

&НаСервере
Процедура ВариантыСтрахованияСуммаПремииПриИзмененииНаСервере(ПараметрыДействия=Неопределено)
	
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	Документы.РабочийЛистОтделаСтрахования.ВариантыСтрахованияСуммаПремииПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры 

&НаКлиенте
Процедура ВариантыСтрахованияСуммаПремииПриИзменении(Элемент)
	
	ВариантыСтрахованияСуммаПремииПриИзмененииНаСервере();
...
&НаКлиенте
Процедура ВвестиСтраховойПолис(Команда)
	
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Пожалуйста, запишите текущий документ.'"));
		Возврат;
	КонецЕсли;
	
	ТекущаяСтрока = Элементы.ВариантыСтрахования.ТекущиеДанные;
	Если ТекущаяСтрока = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выберите строку страхования'"),Объект.Ссылка);
		Возврат;
	Иначе
		ТекущийСтраховщик           = ТекущаяСтрока.Страховщик;
		ТекущийПрограммаСтрахования = ТекущаяСтрока.ПрограммаСтрахования;
		ТекущийВидСтрахования       = ТекущаяСтрока.ВидСтрахования;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ТекущийСтраховщик) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните страховщика текущей строки'"),Объект.Ссылка);
		Возврат;
	ИначеЕсли  НЕ ЗначениеЗаполнено(ТекущийПрограммаСтрахования) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните программу страхования текущей строки'"),Объект.Ссылка);
		Возврат;
	ИначеЕсли  НЕ ЗначениеЗаполнено(ТекущийВидСтрахования) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните вид страхования в текущей строки'"), Объект.Ссылка);
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура();
	
	ДокументСтраховойПолис = ПолучитьСтраховойПолис();
	
	// Если подходящего документа не имеется в базе
	Если ДокументСтраховойПолис = Неопределено Тогда
		СтруктураПараметров = Новый Структура();
		СтруктураПараметров.Вставить("Основание",            Объект.Ссылка);
		СтруктураПараметров.Вставить("Страховщик",           ТекущийСтраховщик);
		СтруктураПараметров.Вставить("ПрограммаСтрахования", ТекущийПрограммаСтрахования);
		СтруктураПараметров.Вставить("ВидСтрахования",       ТекущийВидСтрахования);
		СтруктураПараметров.Вставить("СуммаПремии",          ТекущаяСтрока.СуммаПремии);
		ПараметрыФормы.Вставить("ЗначенияЗаполнения", СтруктураПараметров);
	Иначе
		ПараметрыФормы.Вставить("Ключ", ДокументСтраховойПолис);
	КонецЕсли;
	
	ОткрытьФорму("Документ.СтраховойПолис.ФормаОбъекта", ПараметрыФормы, ЭтотОбъект);
...
Процедура ОбновлятьКИ(Команда)
	...
	Если ИмяКоманды = "ОбновлятьКИ" И ПроверитьСогласиеНаОбработкуПерсональныхДанных(Объект.Страхователь) Тогда
...
Процедура НаписатьПисьмо(Команда)
	...
	Объект.АдресЭлектроннойПочты, ВидКонтактнойИнформацииЭлектроннойПочты, Объект.Страхователь);
...
&НаСервере
Функция ПолучитьСтраховойПолис()
	
	Результат = Неопределено;
	// Получим данные текущей строки табличной части
	ТекущиеДанные = Объект.ВариантыСтрахования.НайтиПоИдентификатору(Элементы.ВариантыСтрахования.ТекущаяСтрока);
	
	ЗапросСтраховогоПолиса = Новый Запрос();
	ЗапросСтраховогоПолиса.Текст = "ВЫБРАТЬ
	                              |	СтраховойПолис.Ссылка КАК Ссылка
	                              |ИЗ
	                              |	Документ.СтраховойПолис КАК СтраховойПолис
	                              |ГДЕ
	                              |	СтраховойПолис.ДокументОснование = &Ссылка
	                              |	И СтраховойПолис.Страховщик = &Страховщик
	                              |	И СтраховойПолис.ПрограммаСтрахования = &ПрограммаСтрахования
	                              |	И СтраховойПолис.ВидСтрахования = &ВидСтрахования";
	ЗапросСтраховогоПолиса.УстановитьПараметр("Ссылка",               Объект.Ссылка);
	ЗапросСтраховогоПолиса.УстановитьПараметр("Страховщик",           ТекущиеДанные.Страховщик);
	ЗапросСтраховогоПолиса.УстановитьПараметр("ПрограммаСтрахования", ТекущиеДанные.ПрограммаСтрахования);
	ЗапросСтраховогоПолиса.УстановитьПараметр("ВидСтрахования",       ТекущиеДанные.ВидСтрахования);
	
	ВыборкаСтраховыхПолисов = ЗапросСтраховогоПолиса.Выполнить().Выбрать();
	
	Если ВыборкаСтраховыхПолисов.Следующий() Тогда
		Результат = ВыборкаСтраховыхПолисов.Ссылка;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбработатьИзменениеСтрахователя()
	...
	КонецЕсли;
	СтраховательПриИзмененииНаСервере(ПараметрыДействия);
	ОбработкаРезультатаВыполненияДействия(ПараметрыДействия);
