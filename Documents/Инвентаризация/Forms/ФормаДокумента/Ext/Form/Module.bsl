...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		РежимПолученияЦеныПоСебестоимости = Истина;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	РаботаСФормойКлиент.ЗамерВремениЗапись("Инвентаризация", ПараметрыЗаписи.РежимЗаписи,
...
Процедура ПриСохраненииДанныхВНастройкахНаСервере(Настройки)
	...
	Настройки.Вставить("Инвентаризация_ЗаполнятьПоСебестоимости", РежимПолученияЦеныПоСебестоимости);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Настройки.Получить("Инвентаризация_ЗаполнятьПоСебестоимости")=ИСТИНА Тогда
		РежимПолученияЦеныПоСебестоимости = Истина;
	КонецЕсли;
...
Процедура СкладКомпанииПриИзменении(Элемент)
	...
	ПараметрыДействия = Новый Структура;
	ПараметрыДействия.Вставить("РежимПолученияЦеныПоСебестоимости", РежимПолученияЦеныПоСебестоимости);
...
&НаКлиенте
Процедура РежимПолученияЦеныПоСебестоимостиПриИзменении(Элемент)
...
Процедура ТоварыГТДИзлишковПриИзменении(Элемент)
	...
	Если Элементы.Товары.ТекущиеДанные.Количество <= 0 И ЗначениеЗаполнено(Элементы.Товары.ТекущиеДанные.ГТДИзлишков) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтрШаблон(НСтр("ru='Строка № %1 . ГТД используется для излишков.'"),Элементы.Товары.ТекущиеДанные.НомерСтроки)
		);
...
Процедура УправлениеДиалогомНаСервере()
	...
	Элементы.ТоварыПартия.Видимость = ПраваИНастройкиПользователя.Значение("ВыборочноеСписаниеПартий", Объект) И (НЕ Комиссия) И Документы.Инвентаризация.ПолучитьФлагПолучатьКнижноеКоличествоИзПартий();
...
Процедура ОбработкаРезультатаОповещенияНаСервере(РезультатОповещения, ДополнительныеПараметры=Неопределено)
	...
	// Произведем передачу дополнительных параметров заполнения
	Если ТипЗнч(ДополнительныеПараметры)=Тип("Строка") И Лев(ДополнительныеПараметры,17)="КомандаЗаполнения" Тогда
		РезультатОповещения.ПараметрыДействия.Вставить("РежимПолученияЦеныПоСебестоимости", РежимПолученияЦеныПоСебестоимости);
	ИначеЕсли ДополнительныеПараметры="ПараметрыДокумента" Тогда
		РезультатОповещения.Вставить("РежимПолученияЦеныПоСебестоимости", РежимПолученияЦеныПоСебестоимости);
	КонецЕсли;
...
Процедура ШтрихкодированиеОбработкаОповещенияНаСервере(РезультатОповещения, ДополнительныеПараметры)
	...
	РезультатОповещения.Вставить("ИмяРеквизитаКоличество", "КоличествоФакт");
