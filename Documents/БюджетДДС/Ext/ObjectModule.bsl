...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(СценарийПланирования) ИЛИ НЕ ЗначениеЗаполнено(СценарийПланирования.Периодичность) Тогда
		...
		"ru = 'Не выбран сценарий планирования, либо у выбранного сценария не выбрана периодичность планирования.'"), 
		ЭтотОбъект,,, Отказ);
	КонецЕсли;
	
	// проверим правильность введенного подразделения хоз.операции.
	Если НЕ ПланированиеСервер.ПроверитьКорректностьВыбораПодразделенияИХозОперации(ХозОперация, 
																				ПодразделениеКомпании) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр(
		"ru = 'Для данного подразделения нельзя вводить данный вид бюджета.'"), ЭтотОбъект,,, Отказ);
	КонецЕсли;
	...
	Если ИспользоватьРаздельныеГрафикиПогашения Тогда
		...
		Для Каждого ТекСтр Из тзВрем Цикл
			...
			Если ТекСтр.Уникальность > 1 Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'У статьи <%1> период <%2> не уникален.'"),
					ТекСтр.СтатьяДДС,ПланированиеСервер.ПолучитьДатыПланируемогоПериода(Параметры));
		...
		Для Каждого ТекСтр Из тзВрем Цикл
			Если ТекСтр.Процент > 100 Тогда 
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
						"ru = 'У статьи <%1> сумма процентов по всем периодам превышает 100%.'"), ТекСтр.СтатьяДДС);
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,ЭтотОбъект,,, Отказ);
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		Для Каждого ТекСтр Из тзВрем Цикл
			...
			Если ТекСтр.Уникальность > 1 Тогда
				...
				Параметры.Вставить("Действие", ТекСтр.Период);
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Период <%1> не уникален.'"),
		...
		Если ИтогПроцент > 100 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Сумма процентов по всем периодам превышает 100%'"),
				ЭтотОбъект,,, Отказ);
