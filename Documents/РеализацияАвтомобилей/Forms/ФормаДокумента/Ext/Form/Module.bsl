///////////////////////////////////////////////////////////////////////////////
// Модуль основной формы документа "Реализация автомобилей"
//
///////////////////////////////////////////////////////////////////////////////
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьПереключательСворачиватьОборудование();
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	РаботаСФормойКлиент.ЗамерВремениЗапись("РеализацияАвтомобилей", ПараметрыЗаписи.РежимЗаписи,
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// Если есть автомобили с обязательными сервисными кампаниями, запретим проведение
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Для Каждого СтрокаТЧ Из Объект.Автомобили Цикл
			СервисныеКампании = АвтомобилиСервер.ПроверитьСервиснуюКампанию(СтрокаТЧ.Автомобиль, ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса()), Истина);
			СервисныеКампании = СервисныеКампании.ВыгрузитьКолонку("СервиснаяКампания");
			Для Каждого Кампания Из СервисныеКампании Цикл
				Если Кампания.ЗапретОтгрузкиБезВыполненияСервиснойКампании = Ложь Тогда
					Продолжить;
				КонецЕсли;
				
				ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Автомобили",СтрокаТЧ.НомерСтроки, "Автомобиль");
				ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон("Для автомобиля <%1> найдена невыполненная обязательная сервисная кампания <%2>. Отгрузка автомобиля запрещена.",
				СтрокаТЧ.Автомобиль,
				Кампания
				),
				ТекущийОбъект,
			...
			КонецЦикла;
		КонецЦикла;
		ПроверитьНаличиеПовреждений(ПараметрыЗаписи);
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого Строка Из Объект.Автомобили Цикл
		Если ЗначениеЗаполнено(Строка.Автомобиль) И НЕ ЗначениеЗаполнено(Строка.Автомобиль.Продавец) Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка записи автомобиля'"),
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ОповеститьПользователя", Сообщение) И ЗначениеЗаполнено(Сообщение) Тогда
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.ДатаПриИзменении(Объект, ПараметрыДействия);
	УправлениеДиалогомНаСервере();
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура СкладКомпанииПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.СкладКомпанииПриИзменении(Объект,ПараметрыДействия);
	УправлениеДиалогомНаСервере();
...
Процедура КонтрагентПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.КонтрагентПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДоговорВзаиморасчетовПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.ДоговорВзаиморасчетовПриИзменении(Объект, ПараметрыДействия);
...
&НаКлиенте
Процедура СворачиватьОборудованиеПриИзменении(Элемент)
	
	Объект.НеВключатьВСтоимостьАвтомобиляСтоимостьОборудования = 
		НЕ Объект.НеВключатьВСтоимостьАвтомобиляСтоимостьОборудования;
...
Процедура АвтомобилиАвтомобильПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.АвтомобилиАвтомобильПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	УправлениеДиалогомНаСервере();
...
Процедура АвтомобилиЦенаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Автомобили.НайтиПоИдентификатору(Элементы.Автомобили.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.АвтомобилиЦенаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура АвтомобилиСуммаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Автомобили.НайтиПоИдентификатору(Элементы.Автомобили.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.АвтомобилиСуммаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура АвтомобилиСуммаВсегоПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Автомобили.НайтиПоИдентификатору(Элементы.Автомобили.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.АвтомобилиСуммаВсегоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура АвтомобилиСтавкаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Автомобили.НайтиПоИдентификатору(Элементы.Автомобили.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.АвтомобилиСтавкаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура АвтомобилиЗаказНаАвтомобильПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Автомобили.НайтиПоИдентификатору(Элементы.Автомобили.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.АвтомобилиЗаказНаАвтомобиль(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура АвтомобилиАвтомобильБезЗаказаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Автомобили.НайтиПоИдентификатору(Элементы.Автомобили.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.АвтомобилиАвтомобильБезЗаказа(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура АвтомобилиПриобретенУФизЛицаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Автомобили.НайтиПоИдентификатору(Элементы.Автомобили.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.АвтомобилиПриобретенУФизЛицаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.ТоварыНоменклатураПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыХарактеристикаНоменклатурыПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыХарактеристикаНоменклатурыПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыЦенаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыЦенаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыСуммаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаВсегоПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыСуммаВсегоПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСтавкаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыСтавкаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаНДСПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыСуммаНДСПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаСкидкиСтрокиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыСуммаСкидкиСтрокиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСуммаСкидкиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыСуммаСкидкиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыСкидкаНаТоварПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.ТоварыСкидкаНаТоварПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ТоварыПроцентСкидкиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыПроцентСкидкиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	ТекущиеДанные.ЗначениеСкидкиШапкиИзменено = Истина;
...
Процедура ТоварыПроцентСкидкиСтрокиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(Элементы.Товары.ТекущаяСтрока);
	Документы.РеализацияАвтомобилей.ТоварыПроцентСкидкиСтрокиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	ТекущиеДанные.ЗначениеСкидкиСтрокиИзменено = Истина;
...
Процедура ЗачетАвансовДокументАвансаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.РеализацияАвтомобилей.ЗачетАвансовДокументАвансаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура СоздатьПодтверждающийДокумент(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда  
		...
		ПараметрыФормы = ПолучитьПараметрыЗаполнения(ТекущиеДанные.ИдентификаторАвтомобиля, ТекущиеДанные.Автомобиль); 
		ПараметрыФормы.СуммаДокумента = ПараметрыФормы.СуммаДокумента + ТекущиеДанные.СуммаВсего;
		ОткрытьФорму("Справочник.ПодтверждающиеДокументы.ФормаОбъекта", ПараметрыФормы);	
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выберите автомобиль.'"));	
    КонецЕсли;
КонецПроцедуры 

&НаСервере
Функция ПолучитьПараметрыЗаполнения(Идентификатор, Автомобиль)
	
	ПараметрыЗаполнения = Новый Структура();
	
	ПараметрыЗаполнения.Вставить("ВидПодтверждающегоДокумента", Перечисления.ВидыДокументов.ПТС); 
	ПараметрыЗаполнения.Вставить("Владелец", Автомобиль);  
    ...
    СуммаОборудования = 0;
	ОборудованиеАвтомобиля = Объект.Товары.НайтиСтроки(Новый Структура("ИдентификаторАвтомобиля",Идентификатор));
	Для каждого СтрокаОборудования Из ОборудованиеАвтомобиля Цикл
		СуммаОборудования = СуммаОборудования + СтрокаОборудования.СуммаВсего;
	КонецЦикла;	 
	
	ПараметрыЗаполнения.Вставить("СуммаДокумента", СуммаОборудования);
...
&НаСервере
Процедура ПроверитьНаличиеПовреждений(ПараметрыЗаписи)
	
	ЭлементыСообщения = Новый Массив;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Срез", Объект.Дата);
	Запрос.УстановитьПараметр("Автомобили", Объект.Автомобили.Выгрузить(,"Автомобиль").ВыгрузитьКолонку("Автомобиль"));
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПоврежденияАвтомобилейСрезПоследних.Автомобиль КАК Автомобиль,
	|	МИНИМУМ(ПоврежденияАвтомобилейСрезПоследних.ПоврежденияУстранены) КАК ПоврежденияУстранены,
	|	МАКСИМУМ(ПоврежденияАвтомобилейСрезПоследних.ДатаУстраненияПлан) КАК ДатаУстраненияПлан,
	|	ПРЕДСТАВЛЕНИЕ(ПоврежденияАвтомобилейСрезПоследних.Автомобиль) КАК АвтомобильПредставление
	|ИЗ
	|	РегистрСведений.ПоврежденияАвтомобилей.СрезПоследних(
	|		&Срез,
	|		Автомобиль В (&Автомобили)) КАК ПоврежденияАвтомобилейСрезПоследних
	|ГДЕ
	|	ПоврежденияАвтомобилейСрезПоследних.ПоврежденияУстранены = ЛОЖЬ
	|
	|СГРУППИРОВАТЬ ПО
	|	ПоврежденияАвтомобилейСрезПоследних.Автомобиль";
	
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		ЭлементыСообщения.Добавить(НСтр("ru = 'В документе присутствуют поврежденные автомобили:'"));
		
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			ЭлементыСообщения.Добавить(
				СтрШаблон(НСтр("ru = '%1 устранить повреждения планируется до %2'"),
					Выборка.АвтомобильПредставление,
					Формат(Выборка.ДатаУстраненияПлан, "ДЛФ=D; ДП='Не указано'")));
		КонецЦикла;
	КонецЕсли;
	
	Если ЭлементыСообщения.Количество() > 0 Тогда
		Сообщение = СтрСоединить(ЭлементыСообщения, Символы.ПС + "    ");
		ПараметрыЗаписи.Вставить("ОповеститьПользователя", Сообщение);
	КонецЕсли;
...
Процедура УправлениеДиалогомНаСервере()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"АвтомобилиПриобретенУФизЛица",
		"Видимость",
		Объект.ХозОперация = Справочники.ХозОперации.РеализацияАвтомобилей
	);
...
&НаКлиенте
Процедура УстановитьПереключательСворачиватьОборудование()

	СворачиватьОборудование = НЕ Объект.НеВключатьВСтоимостьАвтомобиляСтоимостьОборудования;
