...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	ВзаимодействияАльфаАвто.ПриСозданииНаСервере(ЭтотОбъект, Элементы.НаКонтроле);
...
Процедура ТекстСообщенияИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	...
	// Начало АльфаАвто
	// При выполнении обработчика ИзменениеТекстаРедактирования нельзя использовать серверные методы,
	// а также изменять свойства, которые могут привести к серверному вызову.
...
// АльфаАвто
&НаСервере
Процедура ПроверитьСогласиеРассылкиSMS(СписокКонтактов = Неопределено)
	
	Если СписокКонтактов = Неопределено Тогда
		СписокКонтактов = Объект.Адресаты.Выгрузить().ВыгрузитьКолонку("Контакт");
	КонецЕсли;
	
	// Избавимся от пустых контактов
	СписокКонтрагентов = Новый Массив;
	Для Каждого ТекущийКонтакт Из СписокКонтактов Цикл
		Если НЕ ЗначениеЗаполнено(ТекущийКонтакт) ИЛИ ТипЗнч(ТекущийКонтакт) <> Тип("СправочникСсылка.Контрагенты") Тогда
			Продолжить;
		КонецЕсли;
		СписокКонтрагентов.Добавить(ТекущийКонтакт);
	КонецЦикла;
	...
	ДанныеКонтрагентов = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(СписокКонтрагентов, "СогласиеНаПолучениеSMS,Наименование");
	
	Для Каждого ТекущийКонтакт Из ДанныеКонтрагентов Цикл
		Если ТекущийКонтакт.Значение.СогласиеНаПолучениеSMS Тогда
			Продолжить;
		КонецЕсли;
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(
				НСтр("ru = 'Контрагент %1 не давал согласие на получение смс'"), 
				ТекущийКонтакт.Значение.Наименование
			)
		);
	КонецЦикла;
...
&НаКлиенте
Процедура ТекстСообщенияОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	ОсталосьСимволов = ВзаимодействияКлиентСервер.СформироватьИнформационнуюНадписьКоличествоСимволовСообщений(
	                   Объект.ОтправлятьВТранслите,
	                   Текст);
