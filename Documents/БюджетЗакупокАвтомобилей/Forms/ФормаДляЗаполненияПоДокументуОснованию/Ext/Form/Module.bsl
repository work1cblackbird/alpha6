...
Процедура ДокументОснованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ХозОперация = ПредопределенноеЗначение("Справочник.ХозОперации.БюджетЗакупокПоМоделям") Тогда
		ОтборПоХозОперации = Новый Массив;
		ОтборПоХозОперации.Добавить(ПредопределенноеЗначение("Справочник.ХозОперации.БюджетЗакупокПоМоделям"));
	...
	Иначе
		...
		ОтборПоХозОперации.Добавить(ПредопределенноеЗначение("Справочник.ХозОперации.БюджетЗакупокПоМоделям"));
		ОтборПоХозОперации.Добавить(ПредопределенноеЗначение("Справочник.ХозОперации.БюджетПродажПоМоделям"));
		ОтборПоХозОперации.Добавить(ПредопределенноеЗначение("Справочник.ХозОперации.БюджетЗакупокПоВариантамКомплектации"));
...
Процедура УправлениеОтображением(Элемент = "")
	...
	Если Элемент="ДокументОснование" Тогда
		...
		ИначеЕсли (ДокументОснование.ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоМоделям 
					ИЛИ ДокументОснование.ХозОперация = Справочники.ХозОперации.БюджетПродажПоМоделям)
			       		И ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации Тогда
...
Процедура ЗаполнитьТабличноеПоле()
	...
	ДокументПоВариантамКомплектации  = (ХозОперация = Справочники.ХозОперации.БюджетПродажПоВариантамКомплектации ИЛИ ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации);
	ОснованиеПоВариантамКомплектации = (ДокументОснование.ХозОперация = Справочники.ХозОперации.БюджетПродажПоВариантамКомплектации ИЛИ ДокументОснование.ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации);
...
Процедура ОбработкаРеквизита(Данные, Реквизит = "")
	...
	Если Реквизит = "Автомобили.Автомобиль" Тогда
		...
		Данные.Цена = ЦенообразованиеСервер.ПолучитьЦенуАвтомобиля(
			ТипЦен, Данные.Автомобиль,, Дата, Константы.ВалютаУправленческогоУчетаКомпании.Получить());
	...
	ИначеЕсли Реквизит="Автомобили.СуммаВсегоУпрНеРаспред" Тогда
		ХОПоВариантамКомплектации = (ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации);
...
Процедура ЗаполнениеПростымСпособом()
	...
	ОбработчикОповещения = Новый ОписаниеОповещения("ВыводРезультатаЗаполненияНоменлатуры", ЭтотОбъект);
	
	ОткрытьФорму("Документ.БюджетЗакупокАвтомобилей.Форма.ФормаДляЗаполненияПростымСпособом", ПараметрыФормы, ЭтотОбъект, ,,, ОбработчикОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
//
&НаСервере
Процедура ВыводРезультатаЗаполненияНоменлатурыНаСервере(ДанныеАвтомобилей)
...
//
&НаКлиенте
Процедура ВыводРезультатаЗаполненияНоменлатуры(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ВыводРезультатаЗаполненияНоменлатурыНаСервере(РезультатВыбора.ТаблицаАвтомобилей);
...
Процедура СформироватьИтогововуюТаблицуАвтомобилей()
	...
	ДвухУровневоеДерево = (ДокументОснование.ХозОперация = Справочники.ХозОперации.БюджетПродажПоМоделям И ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации);
	ДвухУровневоеДерево = ДвухУровневоеДерево ИЛИ (ДокументОснование.ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоМоделям И ХозОперация = Справочники.ХозОперации.БюджетЗакупокПоВариантамКомплектации);
	ДатаДокумента       = Дата;
...
Процедура РаспределитьСуммыМоделейПоВариантамКомплектации(Параметры)
	...
	ПараметрыРаспределения.Вставить("ЕстьСебестоимостьУпр",             Ложь);
