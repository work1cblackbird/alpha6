...
Процедура График(Команда)
	...
	ОткрытьФорму("Документ.БюджетЗакупокАвтомобилей.Форма.ФормаРезультата", ПараметрыФормы, ЭтотОбъект, СчетчикДиаграмм);
...
Функция ПолучитьДополнительныеПараметрыЗапроса()
	...
	Иначе
		...
		ЭлементОтбора = ДеревоОтбора.Строки.Найти("ПодразделениеЗакупки", "ЛевоеЗначение");
		Если ЭлементОтбора.Использование Тогда
			СтрокаУсловия = СтрЗаменить(СтрЗаменить(Документы.БюджетЗакупокАвтомобилей.СформироватьСтрокуВидаСравнения(ЭлементОтбора.ВидСравнения),"#","ТаблицаДанных.СкладКомпании.ПодразделениеКомпании"),"&","&Подразделение");
			ДополнительныеПараметры.Вставить("ОтборПоПодразделению", СтрокаУсловия);
			ДополнительныеПараметры.Вставить("Подразделение", ЭлементОтбора.ПравоеЗначение);
		КонецЕсли;
...
Процедура Сформировать()
	...
	ИмяРегистра = ?(ТипАнализа=Перечисления.ТипыАнализа.ПоФактическимДаннымПродаж, "ПродажиАвтомобилей", "ОстаткиАвтомобилей");
	...
	СтруктураПолей.Вставить("Цена");
		...
		|
		|ИЗ
		|	РегистрНакопления."+ИмяРегистра+" КАК ТаблицаДанных
