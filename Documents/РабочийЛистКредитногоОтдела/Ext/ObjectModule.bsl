// Модуль объекта документа "Рабочий лист кредитного отдела"
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения="", СтандартнаяОбработка=ИСТИНА) Экспорт
	...
	Если ДанныеЗаполненияКонтрагент <> Неопределено Тогда
		Клиент = ДанныеЗаполненияКонтрагент;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства, "ОбновлятьКИ", Ложь)
		И ТипЗнч(Клиент) = Тип("СправочникСсылка.Контрагенты") Тогда
		
		КомментарийТелефона = УправлениеКонтактнойИнформацией.КомментарийКонтактнойИнформации(ПредставлениеТелефонаСтрокой);
		УправлениеКонтактнойИнформациейАльфаАвто.СохранитьТелефон(Клиент, ПредставлениеТелефона, КомментарийТелефона, Отказ);
		УправлениеКонтактнойИнформациейАльфаАвто.СохранитьЭлектроннуюПочту(Клиент, АдресЭлектроннойПочты, Отказ);
...
Функция ОбработкаЗаполнения_Событие(ДанныеЗаполнения, ТекстЗаполнения = "", СтандартнаяОбработка = Истина) Экспорт
	...
	// заполним значение реквизита Клиент контрагентом из основания,
	// иначе заполним Клиента значением реквизита ОбращениеККлиенту документа-основания.
	Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "Контрагент");
	Если ЗначениеЗаполнено(Контрагент) Тогда
		Клиент = Контрагент;
		Документы.РабочийЛистКредитногоОтдела.КлиентПриИзменении(ЭтотОбъект);
	Иначе
		Клиент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения, "ОбращениеККлиенту");		
	КонецЕсли;
...
Функция ОбработкаЗаполнения_РабочийЛист(ДанныеЗаполнения, ТекстЗаполнения = "", СтандартнаяОбработка = Истина) Экспорт
	...
	ДатаСделки               = ДанныеЗаполнения.Дата;
	Клиент                   = ДанныеЗаполнения.Контрагент;
	МенеджерКредитногоОтдела = ДанныеЗаполнения.Менеджер;
	СтоимостьАвтомобиля      = ДанныеЗаполнения.ЦенаАвтомобилей;
