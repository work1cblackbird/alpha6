// Модуль объекта документа "Изменение цен опций"
...
Функция ОпцииДокумента(ДокументСсылка)
	
	Автомобили = Неопределено;
	
	Если ЕстьРеквизит(ДокументСсылка, "Автомобиль", "Автомобили") Тогда
		Автомобили = ДокументСсылка.Автомобили.ВыгрузитьКолонку("Автомобиль");
	ИначеЕсли ЕстьРеквизит(ДокументСсылка, "Автомобиль") Тогда
		Автомобили = Новый Массив;
		Автомобили.Добавить(ДокументСсылка.Автомобиль);
	КонецЕсли;
	
	// получим доступные опции
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОпцииАвтомобилей.Автомобиль.Модель              КАК Модель,
	|	ОпцииАвтомобилей.Автомобиль.ВариантКомплектации КАК ВариантКомплектации,
	|	ОпцииАвтомобилей.Опция                          КАК Опция
	|ИЗ
	|	РегистрСведений.ОпцииАвтомобилей КАК ОпцииАвтомобилей
	|ГДЕ
	|	ОпцииАвтомобилей.Автомобиль В(&Автомобили)
...
Функция ОбработкаЗаполнения_ПоступлениеАвтомобилей(ДанныеЗаполнения, ТекстЗаполнения = "", СтандартнаяОбработка = Истина) Экспорт
	...
	Опции.Загрузить(ОпцииДокумента(ДанныеЗаполнения));
	
	Для Каждого СтрокаОпции Из Опции Цикл
		Документы.ИзменениеЦенОпций.ЗаполнитьБазовуюЦену(ЭтотОбъект, СтрокаОпции);
		Документы.ИзменениеЦенОпций.ОпцииПроцентНаценкиПриИзменении(ЭтотОбъект, СтрокаОпции, ЭтотОбъект.ДополнительныеСвойства.ПараметрыДействия);	
	КонецЦикла;
