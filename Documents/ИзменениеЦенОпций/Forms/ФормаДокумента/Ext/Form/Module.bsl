///////////////////////////////////////////////////////////////////////////////
// Модуль основной формы документа "Изменение цен опций"
//
///////////////////////////////////////////////////////////////////////////////
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	РаботаСФормойКлиент.ЗамерВремениЗапись("ИзменениеЦенОпций", ПараметрыЗаписи.РежимЗаписи,
		Объект.Опции.Количество() > 50);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого ТекСтрока Из Объект.Опции Цикл
...
Процедура ДатаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.ИзменениеЦенОпций.ДатаПриИзменении(Объект, ПараметрыДействия);
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если Объект.Дата <> Объект.ДатаНачалаДействия Тогда
		...
		ПоказатьВопрос(ОповещениеВопроса, НСтр("ru = 'Дата документа изменена. Сменить дату начала действия цен опций?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	ОбработкаРезультатаВыполненияДействия(ПараметрыДействия);
...
Процедура ХозОперацияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.ИзменениеЦенОпций.ХозОперацияПриИзменении(Объект, ПараметрыДействия);
...
Процедура ТипЦенПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	Документы.ИзменениеЦенОпций.ТипЦенПриИзменении(Объект, ПараметрыДействия);
	ЗаполнитьБазовыеЦены();
	УправлениеДиалогомНаСервере();
...
&НаСервере
Процедура ОпцииПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ОпцииПриИзменении(Элемент)
	
	Если Элемент.ТекущаяСтрока <> Неопределено Тогда
		ОпцииПриИзмененииНаСервере()
	КонецЕсли;
...
Процедура ОпцииОпцияПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Опции.НайтиПоИдентификатору(Элементы.Опции.ТекущаяСтрока);
	Документы.ИзменениеЦенОпций.ОпцииОпцияПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
Процедура ОпцииЦенаПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	...
	ТекущиеДанные = Объект.Опции.НайтиПоИдентификатору(Элементы.Опции.ТекущаяСтрока);
	Документы.ИзменениеЦенОпций.ОпцииЦенаПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
...
&НаСервере
Процедура ОпцииВариантКомплектацииПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	ТекущиеДанные = Объект.Опции.НайтиПоИдентификатору(Элементы.Опции.ТекущаяСтрока);
	Документы.ИзменениеЦенОпций.ОпцииВариантКомплектацииПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры

&НаКлиенте
Процедура ОпцииВариантКомплектацииПриИзменении(Элемент)
	
	ОпцииВариантКомплектацииПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ОпцииМодельПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	ТекущиеДанные = Объект.Опции.НайтиПоИдентификатору(Элементы.Опции.ТекущаяСтрока);	Документы.ИзменениеЦенОпций.ОпцииМодельПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры

&НаКлиенте
Процедура ОпцииМодельПриИзменении(Элемент)
	
	ОпцииМодельПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ОпцииПроцентНаценкиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	ТекущиеДанные = Объект.Опции.НайтиПоИдентификатору(Элементы.Опции.ТекущаяСтрока);
	Документы.ИзменениеЦенОпций.ОпцииПроцентНаценкиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры

&НаКлиенте
Процедура ОпцииПроцентНаценкиПриИзменении(Элемент)
	
	ОпцииПроцентНаценкиПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ОпцииСуммаНаценкиПриИзмененииНаСервере(ПараметрыДействия = Неопределено)
	
	ТекущиеДанные = Объект.Опции.НайтиПоИдентификатору(Элементы.Опции.ТекущаяСтрока);
	Документы.ИзменениеЦенОпций.ОпцииСуммаНаценкиПриИзменении(Объект, ТекущиеДанные, ПараметрыДействия);
	
КонецПроцедуры

&НаКлиенте
Процедура ОпцииСуммаНаценкиПриИзменении(Элемент)
	
	ОпцииСуммаНаценкиПриИзмененииНаСервере();
...
Процедура ЗаполнитьБазовыеЦены()
	...
	Для Каждого Строка Из Объект.Опции Цикл
		Документы.ИзменениеЦенОпций.ЗаполнитьБазовуюЦену(Объект, Строка);
	КонецЦикла;
...
Процедура УстановитьКолонкиРасчетныхЦен()
	...
	Для Каждого ТекСтрока Из ТаблицаНаценокРасчетныхЦен Цикл
		МассивСтарыхЦенКолонки.Добавить("Опции" + ТекСтрока.Имя);
		МассивСтарыхЦенРеквизиты.Добавить("Объект.Опции."+ТекСтрока.Имя);
	КонецЦикла;
	...
	// Заполним новые цены
	Для Каждого ТекСтрока Из ТаблицаНаценокРасчетныхЦен Цикл
		НовыйРеквизит = Новый РеквизитФормы(ТекСтрока.Имя, Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)), "Объект.Опции", ТекСтрока.Заголовок);
	...
	Если МассивНовыхЦенКолонки.Количество()>0 ИЛИ МассивСтарыхЦенКолонки.Количество()>0 Тогда
		...
		Для Каждого ТекСтрока Из МассивНовыхЦенКолонки Цикл
			НоваяКолонка = Элементы.Добавить("Опции"+ТекСтрока.Имя, Тип("ПолеФормы"), Элементы.Опции);
			НоваяКолонка.ПутьКДанным = "Объект.Опции."+ТекСтрока.Имя;
		КонецЦикла;
	КонецЕсли;
	// Пересчет расчетных цен документа
	Для Каждого ТекСтрока Из Объект.Опции Цикл
