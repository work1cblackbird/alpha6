...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	// АльфаАвто
	// LISMAK: статус письма в момент при записи = черновик, поэтому вложения в коде ниже не записываются
	// и не отправляются -> тут возврат возможно лишний.
	//Если Объект.СтатусПисьма <> Перечисления.СтатусыИсходящегоЭлектронногоПисьма.Черновик Тогда
	//	Возврат;
	//КонецЕсли;
	// Конец АльфаАвто
...
// Используется в прикладном функционале альфа-авто.
//
&НаСервере
Процедура ОпределитьСпособРедактированияПисьма() Экспорт
// Альфа-
...
Процедура ОтправитьПисьмоКлиент()
	...
	// АльфаАвто
	// Убрать после исправления ошибки в БСП
	Если Результат.Свойство("ОшибкаПодключения") И Результат.ОшибкаПодключения
		ИЛИ НЕ ПустаяСтрока(Результат.ТекстСообщения) Тогда
	// Конец АльфаАвто
...
Процедура ЗаполнитьШаблонПослеВыбора(ПараметрыПисьма)
	...
	// АльфаАвто
	// В БСП в ОбработатьПереданныеПараметры заполнение текста сообщения только для нового объекта.
	// Мы же сюда попали из заполнения по шаблону.
	Если НЕ Объект.Ссылка.Пустая() Тогда
		УстановитьТекстПисьмаПоПереданнымПараметрам(ПараметрыПисьма);
	КонецЕсли;
	// Конец АльфаАвто
	СформироватьСпискиПолучателейПисьма();
	ОпределитьВидимостьДоступностьЭлементовВЗависимостиОтСтатусаПисьма();
