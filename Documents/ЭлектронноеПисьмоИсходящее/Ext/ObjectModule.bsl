...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		...
		Если ЗначениеЗаполнено(Адресат) Тогда
			СписокКИ = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(
				Адресат,
				Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты, ТекущаяДатаСеанса());
			
			Если ТипЗнч(СписокКИ) = Тип("Строка")
					И НЕ ПустаяСтрока(СписокКИ) Тогда
				СтрокаПолучатель = ПолучателиПисьма.Добавить();
				СтрокаПолучатель.Адрес = СписокКИ;
				СтрокаПолучатель.Контакт = Адресат;
			ИначеЕсли ТипЗнч(СписокКИ) = Тип("СписокЗначений") И СписокКИ.Количество()>0 Тогда
				СтрокаПолучатель = ПолучателиПисьма.Добавить();
		...
		КонецЕсли;
	КонецЕсли;
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И ДанныеЗаполнения.Свойство("Кому") И ПолучателиПисьма.Количество() = 0 Тогда
		ПолучателиПисем = ПолучитьЗначениеПараметраСтруктуры(ДанныеЗаполнения, "Кому");
		Если ТипЗнч(ПолучателиПисем) = Тип("Строка") Тогда
			СтрокаПолучатель = ПолучателиПисьма.Добавить();
			СтрокаПолучатель.Адрес = ДанныеЗаполнения.Кому;
			Контакты = Новый ТаблицаЗначений;
			
			Если Взаимодействия.НайтиПоEmail(СтрокаПолучатель.Адрес, Ложь, Контакты) Тогда
				СписокПолучателейПисьма =?(
						Контакты.Количество() = 1,
						Строка(Контакты[0].Контакт) + " ",
						"")
					+ "<" + ДанныеЗаполнения.Кому + ">; ";
			КонецЕсли;
		ИначеЕсли ТипЗнч(ПолучателиПисем) = Тип("Массив") Тогда
			ЗаполнитьКонтакты(ПолучателиПисем)
		КонецЕсли
	КонецЕсли;
	// Конец АльфаАвто
...
Процедура ЗаполнитьПараметрыИзПисьма(Письмо, ПереноситьОтправителяВПолучатели,
	...
	// АльфаАвто
	|	ЭлектронноеПисьмо.Текст,
	|	ЭлектронноеПисьмо.ТекстHTML,
	// Конец АльфаАвто
	...
	// АльфаАвто
	// Заполним текст письма если пересылаем письмо.
	Если ТипОтвета = "Переслать" Тогда
		Текст     = Выборка.Текст;
