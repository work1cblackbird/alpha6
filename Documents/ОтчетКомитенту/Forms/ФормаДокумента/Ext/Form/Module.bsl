...
Процедура ЗаполнитьСлужебныеРеквизиты()
	...
	ИмяДокумента = ?(
		Объект.ХозОперация = Справочники.ХозОперации.ОтчетАгента,
		"ДокументРеализации",
		"ДокументПоступления");
	
	Для Каждого СтрокаТовара Из Объект.Товары Цикл
		
		Если ЕстьРеквизит(СтрокаТовара[ИмяДокумента], "ДоговорВзаиморасчетов") Тогда
			СпособВеденияВзаиморасчетовДоговоров = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				СтрокаТовара[ИмяДокумента],
...
Процедура ОбработкаРезультатаВыполненияДействия(ПараметрыДействия=Неопределено)
	...
	Если ПолучитьЗначениеПараметраСтруктуры(ПараметрыДействия, "ТребуетсяПересчетВознаграждения", ЛОЖЬ) Тогда
		ПоследовательныеОперацииКлиентСервер.ДобавитьВопросДаНет(КоллекцияОпераций, "ТребуетсяПересчетВознаграждения",
