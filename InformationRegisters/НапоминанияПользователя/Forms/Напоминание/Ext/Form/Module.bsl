...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	Если Не УправлениеДоступом.ЕстьРоль("ДобавлениеИзменениеНапоминанийБезОграничений") Тогда
		
		Элементы.ПолучательПодбор.Видимость = Ложь;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	// АльфаАвто
	Если ПараметрыЗаписи.Свойство("НавигационнаяСсылка") Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// АльфаАвто
	Если ТипЗнч(Пользователь)=Тип("СписокЗначений") И Пользователь.Количество() > 1 Тогда
		
		ЭтоПервыйЭлемент = Истина;
		
		Для Каждого ЭлементСписка Из Пользователь Цикл
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(МенеджерЗаписиНапоминания, ТекущийОбъект);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение.Количество() > 1 Тогда
		...
		Объект.Пользователь = Пользователь[0].Значение;
	...
	ИначеЕсли ВыбранноеЗначение.Количество() = 1 Тогда
		...
		Пользователь = ВыбранноеЗначение[0].Значение;
...
Процедура ПодобратьПользователей(Команда=Неопределено, ПараметрыФормы=Неопределено)
	...
	Если ТипЗнч(Пользователь)=Тип("СписокЗначений") Тогда
		...
		ПараметрыФормы = Новый Структура("ПользователиВыбранные", Пользователь);
		
	ИначеЕсли ЗначениеЗаполнено(Пользователь) Тогда
		
		ПользователиВыбранные = Новый СписокЗначений;
		ПользователиВыбранные.Добавить(Пользователь);
	...
	ОткрытьФорму(
		"РегистрСведений.НапоминанияПользователя.Форма.ПодборПользователей",
		ПараметрыФормы,
		ЭтотОбъект,
		ЭтотОбъект,
		,
