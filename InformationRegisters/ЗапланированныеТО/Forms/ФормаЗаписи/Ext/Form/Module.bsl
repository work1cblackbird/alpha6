...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ЗначениеКопирования") И ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		Запись.Источник = Документы.ЗаказНаряд.ПустаяСсылка();
		Запись.ПредставлениеИсточника = НСтр("ru = 'Ручное заполнение в регистре сведений Запланированные ТО'");
	КонецЕсли;
	
	Если Параметры.Свойство("Автомобиль") Тогда
		Запись.Автомобиль = Параметры.Автомобиль;
	КонецЕсли;
	
	Если Параметры.Свойство("ПробегСледующегоТО") Тогда
		Запись.ПробегСледующегоТО = Параметры.ПробегСледующегоТО;
	КонецЕсли;
	
	Если Параметры.Свойство("ДатаСледующегоТО") Тогда
		Запись.ДатаСледующегоТО = Параметры.ДатаСледующегоТО;
	КонецЕсли;
	
	Если Параметры.Свойство("ПредставлениеИсточника") Тогда
		Запись.ПредставлениеИсточника = Параметры.ПредставлениеИсточника;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	// Для получения результата создания в форме владельце
	Если
		НЕ ЗавершениеРаботы
		...
		И ОписаниеОповещенияОЗакрытии.ДополнительныеПараметры.Свойство("РезультатОповещения")
	Тогда
		ОписаниеОповещенияОЗакрытии.ДополнительныеПараметры.РезультатОповещения = Запись.Автомобиль;
	КонецЕсли;
