...
Функция ИзмеренияНабораЗаписей(ИмяТаблицы, ВсеИзмерения = Ложь) Экспорт
	...
	Если Не ВсеИзмерения Тогда
		// АльфаАвто
		// При выполнении регистрации исключаются общие реквизиты по имени. Не смотря на их спользование в качестве измерения регистра сведений.
		// Из-за этого не попадают измерения совпадающие по именам с общими реквизитами.
		// А если такие измерения ведущие или основные их отсутствие приводит к ошибке регистрации в дальнейшем.
		...
		Для Каждого МетаОбщий Из Метаданные.ОбщиеРеквизиты Цикл
			...
			// Из-за этого не попадают измерения совпадающие по именам с общими реквизитами.
			// А если такие измерения ведущие или основные их отсутствие приводит к ошибке регистрации в дальнейшем.			
			Если ЭтоРегистрСведений И Мета.Измерения.Найти(МетаОбщий.Имя) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			// Конец АльфаАвто
