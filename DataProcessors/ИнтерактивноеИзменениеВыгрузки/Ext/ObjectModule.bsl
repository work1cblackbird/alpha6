		...
		Для Каждого Строка Из ТаблицаДополнительныхИзменений Цикл
			...
			Для Каждого ИмяДобавляемойТаблицы Из ДобавляемыеТаблицы Цикл
				...
				Если Строка.ВыборПериода Тогда
					...
					// АльфаАвто
					// Для Заказ-наряда внесем коррективы для получения документов по дате
					ЭтоЗаказНаряд = (Метаданные.НайтиПоПолномуИмени(ИмяДобавляемойТаблицы) = Метаданные.Документы.ЗаказНаряд);
					Если ДатаНачала <> '00010101' Тогда
						Если ЭтоЗаказНаряд Тогда
							ДобавитьЭлементОтбора(ГруппаОтбора.Элементы, "ОбъектРегистрации.ДатаЗакрытия", ВидСравненияКомпоновкиДанных.БольшеИлиРавно, ДатаНачала);
						Иначе
					...
					Если ДатаОкончания <> '00010101' Тогда
						Если ЭтоЗаказНаряд Тогда
							ДобавитьЭлементОтбора(ГруппаОтбора.Элементы, "ОбъектРегистрации.ДатаЗакрытия", ВидСравненияКомпоновкиДанных.МеньшеИлиРавно, ДатаОкончания);
						Иначе
