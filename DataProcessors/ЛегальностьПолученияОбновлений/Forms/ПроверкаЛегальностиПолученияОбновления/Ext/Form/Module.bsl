...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если
		ОбновлениеИнформационнойБазыСлужебный.РежимОбновленияДанных() <> "НачальноеЗаполнение"
		Или Метаданные.Обработки.ЛегальностьПолученияОбновлений.Макеты.Найти("УсловияРаспространенияПродукта") = Неопределено
	Тогда
	...
	ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "[Правообладатель]", Строка(Метаданные.Поставщик));
	ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "[НаименованиеТОР]", Строка(Метаданные.Синоним));
	// Конец АльфаАвто
