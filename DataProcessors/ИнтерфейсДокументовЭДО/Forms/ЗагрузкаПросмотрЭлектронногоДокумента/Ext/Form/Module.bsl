...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПараметрыФайла = Неопределено И Не МожноЗагрузитьФормат(ПараметрыФайла.Формат) Тогда
		Отказ = Истина;
...
&НаСервереБезКонтекста
Функция МожноЗагрузитьФормат(Знач Формат)
	
	Возврат Не ЗначениеЗаполнено(Формат) Или ЭлектронныеДокументыЭДО.ЭтоСтандартныйФормат(Формат);
