...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ Отказ Тогда
		...
		Если (ЭтотОбъект.ТипЗначения.Типы().Количество() <> 1) И (НЕ ЭтоГруппа) Тогда
			Отказ = Истина;
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Должен быть указан единственный тип возможных значений. Запись элемента <"+СокрЛП(ЭтотОбъект)+"> отменена.'")
			);
		КонецЕсли;
	КонецЕсли;
