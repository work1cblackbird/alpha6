...
Процедура ОбновитьОписаниеСвойствВидовДоступа(ЕстьИзменения = Неопределено, ТолькоПроверка = Ложь) Экспорт
	...
	Попытка
		...
		СтандартныеПодсистемыСервер.ОбновитьПараметрРаботыПрограммы("СвойстваВидовДоступа", 
			СвойстваВидовДоступа, ЕстьТекущиеИзменения);
...
Процедура ПроверитьТип(ВидДоступа, Тип, ВсеТипы, Параметры, ПроверкаТиповГрупп = Ложь, ПроверкаПересечения = Ложь)
	...
	Если ВсеТипы.Получить(Тип) <> Неопределено
		И НЕ (ВидДоступа.Имя = "ПодразделенияКомпании" ИЛИ ВидДоступа.Имя = "Контрагенты") Тогда // -РАРУС
