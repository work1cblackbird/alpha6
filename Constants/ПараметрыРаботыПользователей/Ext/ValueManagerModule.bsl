...
Процедура ОбновитьОбщиеПараметры(ЕстьИзменения = Неопределено, ТолькоПроверка = Ложь) Экспорт
	...
	Попытка
		...
		СтандартныеПодсистемыСервер.ОбновитьПараметрРаботыПрограммы("НазначениеРолей", 
			НазначениеРолей, ЕстьТекущиеИзменения);
		
		Если Не (ТолькоПроверка И ЕстьТекущиеИзменения) Тогда
			СтандартныеПодсистемыСервер.ОбновитьПараметрРаботыПрограммы("ВсеРоли", 
				ВсеРоли, ЕстьТекущиеИзменения);
		КонецЕсли;
