...
//
Функция НастройкиОтраженияПоУмолчанию() Экспорт
...
Процедура ДополнитьИСохранитьНастройкиИзПредопределенногоПрофиля(КлючНастроек, Настройки, СохраненныйПрофильНастроек)
	...
	ПрофильНастроек = ИнтеграцияЭДО.ПрофилиНастроекОтраженияВходящихДокументов().ПервоначальноеЗаполнение;
	ТаблицаПредопределенногоПрофиля = ЭлектронныеДокументыЭДО.ШаблонНастроекОтраженияВУчете(ПрофильНастроек);
	...
	Если ПредопределенныеНастройки.Количество() > 0 Тогда
		...
		МенеджерЗаписи.НеПредлагатьСохранятьНастройки =
			НастройкиОтраженияВУчетеЭДОСлужебный.НеПредлагатьСохранятьНастройки(МенеджерЗаписи.СпособОбработки);
...
Функция НастройкиАктуальны(КлючНастроек, СохраненныеНастройки)
	...
	ДокументИспользуется = ВидыЭлектронныхДокументов.Найти(КлючНастроек.ВидДокумента) <> Неопределено;
