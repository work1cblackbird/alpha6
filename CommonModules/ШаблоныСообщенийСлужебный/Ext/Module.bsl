...
Функция ОпределитьПараметрыТекстаСообщения(ТекстСообщения)
	...
	// АльфаАвто
	// Список параметров требуется обрабатывать в определенной последовательности.
	// Так например параметр [Документ.Организация] требуется обработать после
	// [Документ.Организация.ИНН], т.к. в дереве параметров ИНН должен быть добавлен первым.
	// Поэтому переходим на таблицу значений для возможности сортировки.
	...
	Пока Позиция > 0 Цикл
		...
		Если ПозицияОкончание > 0 Тогда
			...
			СтрокаПараметра.Параметр = НайденныйПараметр;
			// Конец АльфаАвто
	...
	КонецЦикла;
	// АльфаАвто
	ТаблицаПараметров.Сортировать("Параметр Убыв");
	// Конец АльфаАвто
	...
	//		СтрокаФормата = "";
	//	КонецЕсли;
	Для каждого ЭлементаМассиваПараметров Из ТаблицаПараметров Цикл
		ПозицияФормат = СтрНайти(ЭлементаМассиваПараметров.Параметр, "{");
		Если ПозицияФормат > 0 Тогда
			ИмяПараметра  = Лев(ЭлементаМассиваПараметров.Параметр, ПозицияФормат - 1);
			СтрокаФормата = Сред(ЭлементаМассиваПараметров.Параметр, ПозицияФормат );
		Иначе
			ИмяПараметра  = ЭлементаМассиваПараметров.Параметр;
			СтрокаФормата = "";
		КонецЕсли;
	// Конец АльфаАвто
	...
	Иначе
		...
		Иначе
			Если СоответствиеПараметров[МассивРазобранныйПараметр[0] + СтрокаФормата] = Неопределено Тогда
				СоответствиеПараметров.Вставить(МассивРазобранныйПараметр[0] + СтрокаФормата, "");
			// АльфаАвто
			// Если у нас есть некий параметр, а у него уже есть дочерние параметры,
			// считаем что он ссылочный и в дочерние параметры добавляем ссылку.
			ИначеЕсли ТипЗнч(СоответствиеПараметров[МассивРазобранныйПараметр[0] + СтрокаФормата]) = Тип("Строка") Тогда 
				// Ничего не делаем.
			ИначеЕсли СоответствиеПараметров[МассивРазобранныйПараметр[0] + СтрокаФормата].Количество() > 0 Тогда 
				СоответствиеПараметров[МассивРазобранныйПараметр[0] + СтрокаФормата].Вставить("Ссылка", "");
			// Конец АльфаАвто
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыПоПредмету(Реквизиты, Предмет)
	...
	// АльфаАвто
	// Выше были заполнены значения параметров первого уровня.
	// Теперь аналогично по рекурсии заполняем значения вложенных параметров.
	Для Каждого Реквизит Из Реквизиты Цикл 
		
		Если ТипЗнч(Реквизит.Значение) = Тип("Соответствие") Тогда
			
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Предмет, Реквизит.Ключ)
				И ОбщегоНазначения.ЗначениеСсылочногоТипа(Предмет[Реквизит.Ключ]) Тогда
				ЗаполнитьРеквизитыПоПредмету(Реквизит.Значение, Предмет[Реквизит.Ключ]);
			КонецЕсли;
...
Функция ВставитьПараметрыВСтрокуСогласноТаблицеПараметров(Знач ШаблонСтроки, ВставляемыеЗначения, Знач Префикс = "") Экспорт
	...
	Для каждого СписокРеквизитов Из ВставляемыеЗначения Цикл
		...
		Иначе
			...
			// АльфаАвто
			// Если параметр заполнен по ссылке, то в шаблоне сообщения он фигурирует без ".Ссылка".
			// Заполним параметр в шаблоне поиском без ".Ссылка".
			Если СписокРеквизитов.Ключ = "Ссылка" Тогда 
				Результат = СтрЗаменить(Результат, "[" + Лев(Префикс, СтрДлина(Префикс) - 1) + "]", СписокРеквизитов.Значение);
			КонецЕсли;
			// Конец АльфаАвто
		КонецЕсли;
	КонецЦикла;
...
Процедура РеквизитыПоМетаданнымОбъекта(Реквизиты, МетаданныеОбъект, СписокРеквизитов = "", ИсключаяРеквизиты = "", Префикс = "")
	...
	Если НЕ ОбщегоНазначения.ЭтоПеречисление(МетаданныеОбъект) Тогда
		// АльфаАвто
		ШаблоныСообщенийАльфаАвто.ДополнитьОбъектОбщимиРеквизитами(
			МетаданныеОбъект,
			Реквизиты,
			Префикс,
...
Процедура ОбработатьОпределитьРеквизитыДляЗапросаПоМетаданным(ПараметрыОснования, МетаданныеОбъекта)
	...
	Для каждого ПараметрОснования Из ПараметрыОснования Цикл
		...
		ИначеЕсли МетаданныеОбъекта.Реквизиты.Найти(ИмяПараметра) = Неопределено Тогда
			...
			// АльфаАвто
			ШаблоныСообщенийАльфаАвто.ПроверитьОбщийРеквизит(ИмяПараметра, РеквизитНеНайден);
			// Конец АльфаАвто
...
Процедура ЗаполнитьПолучателейСообщения(ПараметрыОтправки, ПараметрыШаблона, Результат, МенеджерОбъекта)
	...
	Если ПараметрыОтправки.Свойство("ДополнительныеПараметры")
		...
		// АльфаАвто
		Если ПредметСообщения.Предмет <> Неопределено Тогда
			ТипПредмет = ТипЗнч(ПредметСообщения.Предмет);
			Если Метаданные.НайтиПоТипу(ТипПредмет) <> Неопределено Тогда 
				МетаданныеОбъекта = ПредметСообщения.Предмет.Метаданные();
			КонецЕсли;
		КонецЕсли;
		// Конец АльфаАвто
	Иначе
		...
		Если ПредметСообщения <> Неопределено Тогда
			ТипПредмет = ТипЗнч(ПредметСообщения.Предмет);
			Если Метаданные.НайтиПоТипу(ТипПредмет) <> Неопределено Тогда 
				МетаданныеОбъекта = ПредметСообщения.Метаданные();
			КонецЕсли;
		КонецЕсли;
		// Конец АльфаАвто
	КонецЕсли;
	...
	// АльфаАвто
	Если МетаданныеОбъекта = Неопределено ИЛИ НЕ ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта) Тогда
		Возврат;
	КонецЕсли;
	// Конец АльфаАвто
