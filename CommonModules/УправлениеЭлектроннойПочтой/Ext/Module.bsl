...
Процедура ЗагрузитьПочтуПользователя(Результат)
	...
	// АльфаАвто
	|	ВЫБОР
	|		КОГДА УчетныеЗаписиЭлектроннойПочты.ВладелецУчетнойЗаписи = ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(НастройкиУчетныхЗаписейЭлектроннойПочты.ОтветственныйЗаОбработкуПисем, ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка))
	|		ИНАЧЕ УчетныеЗаписиЭлектроннойПочты.ВладелецУчетнойЗаписи
	|	КОНЕЦ КАК ОтветственныйЗаОбработкуПисем,
	// Конец АльфаАвто
	|	УчетныеЗаписиЭлектроннойПочты.ОставлятьКопииСообщенийНаСервере КАК ОставлятьКопии,
...
Функция ЗаписатьЭлектронноеПисьмо(ДанныеУчетнойЗаписи, Сообщение, ОтветственныйЗаОбработкуПисем,
	...
	Попытка
		...
		КонецЕсли;
		Если ЕстьРеквизит(Письмо, "ХозОперация") И (НЕ ЗначениеЗаполнено(Письмо.ХозОперация) ИЛИ 
			НЕ Справочники.ХозОперации.ПолучитьСоответствиеХозОперацииОбъекта(Письмо)) Тогда
			Письмо.ХозОперация = Справочники.ХозОперации.ПолучитьХозОперациюПоУмолчанию(Письмо);
		КонецЕсли;
		// Конец АльфаАвто
		Письмо.Записать();
...
Функция ПолучитьУчетнуюЗаписьДляОтправкиПоУмолчанию() Экспорт
	...
	Запрос.УстановитьПараметр("ТекущийСотрудник", Пользователи.ТекущийПользователь().Сотрудник);
	// Конец АльфаАвто
...
Функция ТекстЗапросаПоДоступнымУчетнымЗаписям()
	...
	|		КОГДА УчетныеЗаписиЭлектроннойПочты.ВладелецУчетнойЗаписи = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	// АльфаАвто
	|				И НастройкиУчетныхЗаписейЭлектроннойПочты.ОтветственныйЗаОбработкуПисем = &ТекущийСотрудник
	// Клнец АльфаАвто
	|			ТОГДА 2
	|		ИНАЧЕ 2
