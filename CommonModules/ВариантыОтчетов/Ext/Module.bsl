...
Процедура ЗагрузитьПользовательские(ПользовательскиеВарианты = Неопределено) Экспорт
	...
	Пока ОписаниеВарианта.Следующий() Цикл 
		...
		// АльфаАвто
		ХранилищеВарианта.ТипДоступа = Перечисления.ТипыДоступаКВариантуОтчета.ПоПользователю;
...
Процедура ПриУдаленииПользователя(ПользовательОбъект, Отказ) Экспорт
	...
	// АльфаАвто
	ВариантыОтчетовАльфаАвто.ПриУдаленииПользователя(ПользовательОбъект, Отказ);
	Возврат;
	// Конец АльфаАвто
...
Функция НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	// АльфаАвто
	ЕстьОтборПоТипуДоступа = ПараметрыПоиска.Свойство("ПоТипуДоступа") И ПараметрыПоиска.ПоТипуДоступа;
	ЕстьОтборПоПрототипуКлюча = ПараметрыПоиска.Свойство("ПрототипКлюч");
	// Конец АльфаАвто
	...
	// АльфаАвто
	ПоказыватьЛичныеВариантыОтчетовДругихАвторов = Пользователи.ЭтоПолноправныйПользователь() ИЛИ ЕстьОтборПоТипуДоступа;
	// АльфаАвто
	...
	Если ЕстьОтборПоПрототипуКлюча Тогда
		Запрос.УстановитьПараметр("ПрототипКлюч", ПараметрыПоиска.ПрототипКлюч);
	КонецЕсли;
	// Конец АльфаАвто
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Или ПоискТолькоВариантовБезПодсистем Тогда
		...
		// АльфаАвто
		// Изменим получение текста запроса
		ТекстЗапроса = ВариантыОтчетовАльфаАвто.ТекстЗапросаОтчетовСУточненнымиОтборами(ЕстьОтборПоТипуДоступа);
		//Конец АльфаАвто
	...
	Иначе
		// АльфаАвто
		// Изменим получение текста запроса
		ТекстЗапроса = ВариантыОтчетовАльфаАвто.ТекстЗапросаОтчетовСПростымиОтборами(
			ЕстьОтборПоТипуДоступа,
			ЕстьОтборПоПрототипуКлюча
		);
		// Конец АльфаАвто
...
Процедура СформироватьРезультатыПоиска(Знач МассивСлов, Знач ИсходнаяТаблица, Результат)
	...
	Для Индекс = 1 По Количество Цикл
		...
		// АльфаАвто
		Если Не ЗначениеЗаполнено(Вариант.НаименованиеОтчета.Значение)
			И ЗначениеЗаполнено(СтрокаТаблицы.НаименованиеОтчета) Тогда
		...
		Если Индекс = Количество Или СтрокаТаблицы.Ссылка <> Вариант.Ссылка Тогда
			...
			Для Каждого Слово Из МассивСлов Цикл
				...
				СловоНайдено = ОтметитьСлово(Вариант.НаименованиеВарианта, Слово) 
					...
					Или ОтметитьСлово(Вариант.НаименованиеОтчета, Слово, Истина);
				// КонецАльфаАвто
