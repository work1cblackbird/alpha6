...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено) Экспорт
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		// АльфаАвто
		Если ОписаниеСвойства.Удалено
			И Форма.Свойства_СкрытьУдаленные
			ИЛИ ОписаниеСвойства.ТолькоДляПечати
			И (НЕ ОписаниеСвойства.ЗаполнятьОбязательно) Тогда
			Продолжить;
		КонецЕсли;
		// Конец АльфаАвто
		...
		// АльфаАвто
		Если ОписаниеСвойства.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.Доверенность Тогда
			ДоверенноеЛицо = ОписаниеСвойств.Найти(ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ДоверенноеЛицо, "Свойство");
			Если ДоверенноеЛицо <> Неопределено Тогда
				НоваяСвязь  = Новый СвязьПараметраВыбора("Отбор.Владелец", ДоверенноеЛицо.ИмяРеквизитаЗначение);
				НовыйМассив = Новый Массив();
				НовыйМассив.Добавить(НоваяСвязь);
				Если ЕстьРеквизит(ОписаниеОбъекта, "Контрагент") Тогда
			...
			ИначеЕсли ЕстьРеквизит(ОписаниеОбъекта, "Контрагент") Тогда
				...
				Элемент.СвязиПараметровВыбора = НовыеСвязи;
			КонецЕсли;
			НовыйПараметр = Новый ПараметрВыбора("Отбор.ВидПодтверждающегоДокумента", Перечисления.ВидыДокументов.Доверенность);
			НовыйМассив = Новый Массив();
...
Процедура ЗаполнитьМеткиОбъекта(Форма, Объект = Неопределено, ПроизвольныйОбъект = Ложь) Экспорт
	...
	Если ВариантОтображенияМеток = Перечисления.ВариантыОтображенияМеток.Надпись Тогда
		Для Индекс = 0 По КоличествоМеток - 1 Цикл
			...
			КонецЕсли;
			// АльфаАвто
			НовыйЭлемент = Форма.Элементы.Добавить("Метка" + ГруппаМетки.Имя + Метка.Имя, Тип("ДекорацияФормы"), ГруппаМетки);
			// Конец АльфаАвто
	...
	Если ЗначениеЗаполнено(СкрытьМеток) Тогда
		// АльфаАвто
		НовыйЭлемент = Форма.Элементы.Добавить("ОстальныеМетки"+ГруппаМетки.Имя, Тип("ДекорацияФормы"), ГруппаМетки);
		// Конец АльфаАвто
		НовыйЭлемент.Вид = ВидДекорацииФормы.Надпись;
		НовыйЭлемент.Гиперссылка = Истина;
		НовыйЭлемент.УстановитьДействие("Нажатие", "Подключаемый_СвойстваВыполнитьКоманду"); 
...
Функция ПредставлениеЗначения(Значение, КодЯзыка, ФорматСвойства)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Булево")
		...
		КонецЕсли;
	// АльфаАвто	
	ИначеЕсли ОбщегоНазначения.ЭтоПеречисление(Значение.Метаданные()) ИЛИ ОбщегоНазначения.ЭтоДокумент(Значение.Метаданные()) Тогда
