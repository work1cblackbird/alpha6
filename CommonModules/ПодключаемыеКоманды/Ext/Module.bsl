...
Процедура ВывестиКоманды(Форма, Команды, ПараметрыРазмещения)
	...
	Для Каждого СведенияОПодменюКоманды Из СведенияОВсехПодменю Цикл
		Для Каждого ИндексКоманды Из СведенияОПодменюКоманды.Команды Цикл
			...
			КонецЕсли;
			// АльфаАвто
			// Убрал из условия Форма.ТолькоПросмотр для того, чтобы у команд заполнения взвелся флаг ИзменяетСохраняемыеДанные
			...
			// АльфаАвто
			ВыводитьВПодменюОднуКоманду = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				Команда.ДополнительныеПараметры, "ВыводитьВПодменюОднуКоманду", Истина);
			Если СведенияОПодменюКоманды.КоличествоКоманд = 1 И ТипЗнч(ГруппаФормы.Родитель) <> Тип("ФормаКлиентскогоПриложения") 
				И Не КомандаВидаКоманднаяПанель И Не ВыводитьВПодменюОднуКоманду Тогда
