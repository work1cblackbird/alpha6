...
Процедура ЗаполнитьРеквизитыПоУмолчанию(Объект, ДанныеЗаполнения) Экспорт
	...
	Если ЭтоВзаимодействие Тогда
		...
		// АльфаАвто
		// Реквизит "Ответственный" является аналогом реквизита "Менеджер"
		Если ТипЗнч(Объект.Ответственный) = Тип("СправочникСсылка.Сотрудники") Тогда
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Предмет") И ЗначениеЗаполнено(ДанныеЗаполнения.Предмет) Тогда
			ТипПредмет = ТипЗнч(ДанныеЗаполнения.Предмет);
...
Функция ПолучитьКонтактыПоПредмету(Предмет, ТипыКонтактнойИнформации) Экспорт
	...
	Если ВзаимодействияКлиентСервер.ЯвляетсяПредметом(Предмет)
		...
		Иначе
			Контакты = УправлениеДиалогомСервер.ПолучитьКонтактыВзаимодействий(Предмет);
		КонецЕсли;
		// Конец АльфаАвто
...
//  Строка
//
Функция ШаблонУсловияНаСсылкиКонтактовДляЗапроса() Экспорт
...
// АльфаАвто
// Мы используем функцию в рассылке. Сделаем ее экспортной.
...
//
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма, ТипКИ = Неопределено) Экспорт // АльфаАвто
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		ИмяСписка = "Список_" + ОписаниеКонтакта.Имя;
		ЕстьКолонкаАдрес = УстановитьТекстЗапросаКонтакта(Форма[ИмяСписка], ОписаниеКонтакта, ТипКИ); // АльфаАвто
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если ЕстьКолонкиАдрес[ОписаниеКонтакта.Имя] Тогда
			...
			Если ТипКИ = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
				КолонкаАдрес.Заголовок = НСтр("ru = 'Телефон'");
			Иначе
				КолонкаАдрес.Заголовок = НСтр("ru = 'Адрес'");
			КонецЕсли;
			// АльфаАвто
...
Функция УстановитьТекстЗапросаКонтакта(Список, ОписаниеКонтакта, ТипКИ) // АльфаАвто
	...
	Если ЕстьВидДляСписка Тогда
		...
		ВидыКонтактнойИнформации = УправлениеКонтактнойИнформацией.ВидыКонтактнойИнформацииОбъекта(
			МенеджерОбъекта.ПустаяСсылка(), ТипКИ);
		// Конец АльфаАвто
...
Процедура ОтправитьSMS(Знач СтруктураПараметров = Неопределено) Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОтправкаSMS);
	
	// АльфаАвто
	// Добавлен параметр для определения актуальности отправки смс
	АктуальноДней = 0;
	Если СтруктураПараметров <> Неопределено Тогда
		СтруктураПараметров.Свойство("АктуальноДней", АктуальноДней);
	КонецЕсли;
	...
	|	И СообщениеSMSАдресаты.ИдентификаторСообщения = """"
	|	И ВЫБОР
	|			КОГДА &АктуальноДней = 0
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ РАЗНОСТЬДАТ(СообщениеSMS.Дата, &ТекущаяДата, ДЕНЬ) <= &АктуальноДней
	|					ИЛИ РАЗНОСТЬДАТ(СообщениеSMS.ДатаКогдаОтправить, &ТекущаяДата, ДЕНЬ) <= &АктуальноДней
	|		КОНЕЦ
	|	И ВЫБОР
	...
	Запрос.УстановитьПараметр("АктуальноДней", АктуальноДней);
	// Конец АльфаАвто
...
Процедура ОтразитьРезультатыОтправкиSMSВДокументе(ДокументОбъект, РезультатОтправки) Экспорт
	...
	Для Каждого ОтправленноеСообщение Из РезультатОтправки.ОтправленныеСообщения Цикл
		ЕстьСоответствие = Ложь; // АльфаАвто
		Для Каждого НайденнаяСтрока Из ДокументОбъект.Адресаты.НайтиСтроки(
			...
			ЕстьСоответствие = Истина; // АльфаАвто
		КонецЦикла;
		// АльфаАвто
		Если Не ЕстьСоответствие Тогда
			// для Билайна. Они принимают номера без префикса "+7" или "8". Поэтому будем искать так же,
			// как если номер записан в таком формате.
			Для Каждого Строка Из ДокументОбъект.Адресаты Цикл
				Если Строка.НомерДляОтправки = Прав(ОтправленноеСообщение.НомерПолучателя, СтрДлина(Строка.НомерДляОтправки)) Тогда
					Строка.ИдентификаторСообщения = ОтправленноеСообщение.ИдентификаторСообщения;
					Строка.СостояниеСообщения     = Перечисления.СостоянияСообщенияSMS.ОтправляетсяПровайдером;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		// Конец АльфаАвто
...
Функция ТекстЗапросаСпискаВзаимодействий(ЗначениеОтбора = Неопределено) Экспорт
	...
	|ГДЕ
	|	НЕ ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.Встреча
	|	И НЕ ЖурналДокументовВзаимодействия.Ссылка ССЫЛКА Документ.ЗапланированноеВзаимодействие
	|{ГДЕ
...
Процедура ЗаполнитьПодменюПоТипуВзаимодействия(ГруппаПодменю, Форма) Экспорт
	...
	Для каждого Отбор Из СписокОтборов Цикл
		...
		// АльфаАвто
		ЭлементКнопкаПодменю.Картинка = Отбор.Картинка;
		// Конец АльфаАвто
...
Функция СписокОтборовПоТипуВзаимодействий(ТолькоПочта) Экспорт
	...
	Если Не ТолькоПочта Тогда
		// АльфаАвто
		// Убран неиспользуемый тип взаимодейстаия
