...
Функция РегламентныеЗаданияЗависимыеОтФункциональныхОпций() Экспорт
	...
	// АльфаАвто
	ТехнологическаяПлатформаСервер.ПриОпределенииНастроекРегламентныхЗаданий(Зависимости);
	// Конец АльфаАвто
...
Процедура СформироватьТаблицуРегламентныхЗаданий(Параметры, АдресХранилища) Экспорт
	...
	Если ИдентификаторРегламентногоЗадания = Неопределено Тогда
		...
		Для Каждого Задание Из ТекущиеЗадания Цикл
			...
			Обновляемое.ЕстьФормаНастройки 	= НЕ Метаданные.Обработки.ФормыНастроекРегламентныхЗаданий.Формы.Найти("НастройкаРегламентногоЗадания" + ИмяЗадания) = Неопределено;
