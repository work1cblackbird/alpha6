...
Функция ОписаниеКлючаЗаписи(ТипИлиПолноеИмя) Экспорт
	...
	// АльфаАвто
	// Для перехода на новую версию. После удалить.
	МассивПолейИсключение = Новый Массив;
	МассивПолейИсключение.Добавить("УдалитьПрофиль");
	МассивПолейИсключение.Добавить("УдалитьДоступПоИерархии");
	// Конец АльфаАвто
	...
	Для Каждого Поле Из ВсеПоля Цикл
		...
		// АльфаАвто
		Если МассивПолейИсключение.Найти(Поле) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		// Конец АльфаАвто
