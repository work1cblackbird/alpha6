...
Функция ОткрытьФормуКонтактнойИнформации(Параметры, Владелец = Неопределено, Оповещение = Неопределено) Экспорт
	...
	// АльфаАвто
	Если НЕ Параметры.Свойство("ИзФормыИстории") И ЕстьЗаписиКИ(Параметры) Тогда
		Параметры.Вставить("ВводНовогоАдреса", Истина);
		Параметры.Вставить("ДействуетС", ТекущаяДата());
	КонецЕсли;
	// Конец АльфаАвто
...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	Иначе
		// АльфаАвто
		// Вынесем инициализацию элемента формы из условия для последующей обработки.
		ЭлементФормыПредставление = Форма.Элементы.Найти(Элемент.Имя);
		// Конец АльфаАвто
		...
		// АльфаАвто
		// Не корректно работает заполнение по маске из вызова диалогового окна
		КонтакнтаяИнформацияСМаской = Новый Массив();
		КонтакнтаяИнформацияСМаской.Добавить(ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон"));
		КонтакнтаяИнформацияСМаской.Добавить(ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Факс"));
		
		Если
			КонтакнтаяИнформацияСМаской.Найти(РезультатЗакрытия.Тип) <> Неопределено
			И ЭлементФормыПредставление <> Неопределено
			И ЗначениеЗаполнено(ЭлементФормыПредставление.Маска)
		Тогда
			
			ТекстПредставления = УправлениеКонтактнойИнформациейАльфаАвтоКлиент.ТелефонПоМаске(ТекстПредставления,
				ЭлементФормыПредставление.Маска);
...
// АльфаАвто
Функция ЕстьЗаписиКИ(Параметры)
	
	Результат = Ложь;
	ВидКИ = Параметры.ВидКонтактнойИнформации;
	Отбор = Новый Структура("Вид", ВидКИ);
	Если Параметры.Свойство("КонтактнаяИнформацияОписаниеДополнительныхРеквизитов") Тогда
		НайденныеСтроки = Параметры.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(Отбор);
		Для Каждого НайденаяСтрока Из НайденныеСтроки Цикл
			Если ЗначениеЗаполнено(НайденаяСтрока.Значение) Тогда
				Результат = Истина;
				Прервать;
			КонецЕсли;			
		КонецЦикла;
