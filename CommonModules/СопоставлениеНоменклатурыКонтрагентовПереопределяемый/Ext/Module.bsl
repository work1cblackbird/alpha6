...
Процедура ПриОпределенииСвойствУпаковкиЕдиницыИзмеренияНоменклатуры(Знач Упаковка, Знач Номенклатура, СвойстваУпаковки) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто.ПриОпределенииСвойствУпаковкиЕдиницыИзмеренияНоменклатуры(
		Упаковка, Номенклатура, СвойстваУпаковки);
	// Конец АльфаАвто
...
Процедура НайтиНаименованиеЕдиницыИзмеренияПоКодуОКЕИ(КодОКЕИЕдиницыИзмерения, НаименованиеЕдиницыИзмерения) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто.НайтиНаименованиеЕдиницыИзмеренияПоКодуОКЕИ(
		КодОКЕИЕдиницыИзмерения, НаименованиеЕдиницыИзмерения);
	// Конец АльфаАвто
...
Процедура ВладелецУпаковкиЕдиницыИзмеренияНоменклатура(Знач Упаковка, ВладелецУпаковкиНоменклатура) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто.ВладелецУпаковкиЕдиницыИзмеренияНоменклатура(
...
Процедура БазоваяЕдиницаИзмеренияНоменклатуры(Знач Номенклатура, ЕдиницаИзмерения) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто.БазоваяЕдиницаИзмеренияНоменклатуры(Номенклатура, ЕдиницаИзмерения);
	// Конец АльфаАвто
...
Процедура ПриОпределенииСвойствНоменклатурыИнформационнойБазы(Знач НаборНоменклатуры, СвойстваНоменклатурИБ) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто
...
Процедура ПриОпределенииСтруктурыНоменклатурыИнформационнойБазы(СтруктураНоменклатуры) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто
		.ПриОпределенииСтруктурыНоменклатурыИнформационнойБазы(СтруктураНоменклатуры);
	// Конец АльфаАвто
...
Процедура ЗначенияНаименованийДанныхСопоставленияНоменклатурыБЭД(Знач НаборСсылокНаОбъекты, СоответствиеЗначенийНаименований) Экспорт
	...
	// АльфаАвто
	НоменклатураБЭД = Новый Массив;
	ХарактеристикаНоменклатурыБЭД = Новый Массив;
	
	Для Каждого ТекущаяНоменклатура Из НаборСсылокНаОбъекты Цикл
		
		Если ТипЗнч(ТекущаяНоменклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
			
			НоменклатураБЭД.Добавить(ТекущаяНоменклатура);
			
		ИначеЕсли ТипЗнч(ТекущаяНоменклатура) = Тип("СправочникСсылка.ХарактеристикиНоменклатуры") Тогда
			
			ХарактеристикаНоменклатурыБЭД.Добавить(ТекущаяНоменклатура); 
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если СоответствиеЗначенийНаименований = Неопределено Тогда
		
		СоответствиеЗначенийНаименований = Новый Соответствие;
		
	КонецЕсли;
	
	Если НоменклатураБЭД.Количество() > 0 Тогда
		
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			СоответствиеЗначенийНаименований,
			ОбщегоНазначения.ЗначениеРеквизитаОбъектов(НоменклатураБЭД, "НаименованиеПолное"));
		
	КонецЕсли;
	
	Если ХарактеристикаНоменклатурыБЭД.Количество() > 0 Тогда
		
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			СоответствиеЗначенийНаименований,
			ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ХарактеристикаНоменклатурыБЭД, "Наименование"));
...
Процедура ОпределитьНаименованиеТипаОбъектаСопоставленияНоменклатурыБЭД(Знач СсылкаНаОбъект, НаименованиеТипаОбъекта) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто
		.ОпределитьНаименованиеТипаОбъектаСопоставленияНоменклатурыБЭД(СсылкаНаОбъект, НаименованиеТипаОбъекта);
	// Конец АльфаАвто
...
Процедура ТекстЗапросаПоискаВариантовСопоставленияПоШтрихкодамКомбинаций(ТекстЗапроса) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто
		.ТекстЗапросаПоискаВариантовСопоставленияПоШтрихкодамКомбинаций(ТекстЗапроса);
	// Конец АльфаАвто
...
Процедура ИнициализацияТекстаЗапросаПоискаСопоставленияПоНатуральнымКлючам(ТекстЗапроса) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто
		.ИнициализацияТекстаЗапросаПоискаСопоставленияПоНатуральнымКлючам(ТекстЗапроса);
	// Конец АльфаАвто
...
Процедура ПодобратьВариантыСопоставленияПоНатуральнымКлючам(ТаблицаНоменклатурыКонтрагентов, ДополнительныеПараметры) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто
		.ПодобратьВариантыСопоставленияПоНатуральнымКлючам(ТаблицаНоменклатурыКонтрагентов, ДополнительныеПараметры);
	// Конец АльфаАвто
...
Процедура ПриЗаполненииФормыНоменклатурыПоДаннымКонтрагента(Знач НоменклатураКонтрагента, Форма) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто.ПриЗаполненииФормыНоменклатурыПоДаннымКонтрагента(
...
Процедура ПриЗаполненииФормыХарактеристикиПоДаннымКонтрагента(Знач НоменклатураКонтрагента, Форма) Экспорт
	...
	// АльфаАвто
	СопоставлениеНоменклатурыКонтрагентовАльфаАвто.ПриЗаполненииФормыХарактеристикиПоДаннымКонтрагента(
		НоменклатураКонтрагента, Форма);
	// Конец АльфаАвто
