...
Процедура ПриОпределенииНастроекПечати(Настройки) Экспорт
	...
	// АльфаАвто
	УправлениеПечатьюАльфаАвто.ПриОпределенииНастроекПечати(Настройки);
	// Конец АльфаАвто
...
Процедура ПередОтправкойПоПочте(ПараметрыОтправки, ПараметрыВывода, ОбъектыПечати, ПечатныеФормы) Экспорт
	...
	Если ТипЗнч(ОбъектыПечати) = Тип("Массив")  Тогда
		...
		Если ЕстьРеквизит(ОбъектПечати, "Контрагент") Тогда
			Адресат = ОбъектПечати.Контрагент;
		ИначеЕсли ЕстьРеквизит(ОбъектПечати, "Заказчик") Тогда
			Адресат = ОбъектПечати.Заказчик;
		Иначе
			Возврат;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Адресат) Тогда
			...
			МассивОбъектов.Добавить(Адресат);
			
			КонтактнаяИнформацияОбъекта = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(МассивОбъектов, 
			...
			СписокКИ = Запрос.Выполнить().Выгрузить();
	
			Если СписокКИ.Количество() > 0 Тогда
				ПараметрыОтправки.Получатель = СписокКИ[0].Значение;
			КонецЕсли;
		КонецЕсли;
		    ПараметрыОтправки.Вставить("Предмет", ОбъектыПечати[0]);
	КонецЕсли;
	// Конец АльфаАвто
...
Процедура ПриПолученииПодписейИПечатей(Документы, ПодписиИПечати) Экспорт
	...
	// АльфаАвто
	УправлениеПечатьюПереопределяемыйАльфаАвто.ПриПолученииПодписейИПечатей(Документы, ПодписиИПечати);
	// Конец АльфаАвто
