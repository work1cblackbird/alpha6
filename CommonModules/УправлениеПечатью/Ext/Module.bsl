...
Процедура УстановитьНастройкиКомандПечати(КомандыПечати, Владелец)
	...
	ВыборкаСписокОтключенных = РезультатЗапроса[0].Выбрать();
	...
	Пока ВыборкаСписокОтключенных.Следующий() Цикл
		СписокОтключенных.Вставить(ВыборкаСписокОтключенных.УникальныйИдентификатор, Истина);
	КонецЦикла;     
	...
	Для Каждого КомандаПечати Из КомандыПечати Цикл
		...
		Для Каждого ТекущаяСтрока Из НайденныеСтроки Цикл
			...
			НастройкаВидимости = ТекущаяСтрока.НастройкаВидимости.Получить();
			Если НастройкаВидимости <> Неопределено Тогда
				КомандаПечати.ДополнительныеПараметры.Вставить("НастройкаВидимости", НастройкаВидимости);
				Для Каждого ОтборЭлементов Из НастройкаВидимости.Отбор.Элементы Цикл
					УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, Строка(ОтборЭлементов.ЛевоеЗначение), 
																					ОтборЭлементов.ПравоеЗначение, ОтборЭлементов.ВидСравнения);
				КонецЦикла;
			КонецЕсли;
			Прервать; 
...
Процедура УбратьПодписьИПечать(ТабличныйДокумент, СкрыватьПодписиИПечати = Ложь) Экспорт
	...
	Для Каждого Рисунок Из ТабличныйДокумент.Рисунки Цикл
		Если ЭтоПодписьИлиПечать(Рисунок) Тогда
			// Альфа+
			Рисунок.Картинка = БиблиотекаКартинок.Пустая;
			// Альфа-
...
Функция ПечатныеФормыПоОбъектам(ПечатнаяФорма, ОбъектыПечати) Экспорт
	...
	Для Каждого ОбъектПечати Из ОбъектыПечати Цикл
		...
		Иначе
			ТабличныйДокумент = ПечатнаяФорма.ПолучитьОбласть(Область.Верх, Область.Лево, Область.Низ, Область.Право); // АльфаАвто
...
Функция ОбщиеРеквизитыОбъектов(ТипыОбъектов) Экспорт
	...
	// АльфаАвто
	// Дополним список общими реквизитами, которые включены для текущего типа.
	Для Каждого ОбщийРеквизит Из Метаданные.ОбщиеРеквизиты Цикл
		ОбъектВСоставе = ОбщийРеквизит.Состав.Найти(ОбъектыМетаданных[0].Имя);
		Если ОбъектВСоставе = Неопределено
			ИЛИ (НЕ ОбъектВСоставе.Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать) Тогда
			Продолжить;
		КонецЕсли;
		ОбщийСписокРеквизитов.Добавить(ОбщийРеквизит);
	КонецЦикла;
	// Конец АльфаАвто
