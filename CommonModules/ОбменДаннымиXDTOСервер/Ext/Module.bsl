	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Если КомпонентыОбмена.ПропускатьОбъектыСОшибкамиПроверкиПоСхеме Тогда
			Для Каждого ОбъектСсылка Из КомпонентыОбмена.НеВыгруженныеОбъекты Цикл
				// АльфаАвто
				Если  ТипЗнч(ОбъектСсылка) = Тип("Структура") И ОбъектСсылка.Свойство("Ссылка") Тогда
					ПланыОбмена.ЗарегистрироватьИзменения(УзелДляОбмена, ОбъектСсылка.Ссылка);
				Иначе
...
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, Знач ПравилоКонвертации, Действие = "КонвертироватьИЗаписать") Экспорт
	...
	Если КомпонентыОбмена.УзелКорреспондентаОбъект = Неопределено
		И ЗначениеЗаполнено(КомпонентыОбмена.УзелКорреспондента) Тогда
...
Процедура ПроизвестиЧтениеДанных(КомпонентыОбмена, ТаблицыДляЗагрузки = Неопределено) Экспорт
	...
	// АльфаАвто
	// Выведем сформированные объекты
	Для Каждого ТекущийОбъект Из Результаты.МассивЗагруженныхОбъектов Цикл
		СтрокаСообщения = Нстр("ru = 'План обмена: ""%1"". Объект ""%2"" успешно загружен.'");
		СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				СтрокаСообщения,
				Строка(КомпонентыОбмена.УзелКорреспондента),
