...
Процедура ОткрытьРассылкуИзФормыОтчета(Форма, Ссылка = Неопределено)
	...
	Если РежимВариантаОтчета Тогда
		...
		// АльфаАвто
		// Изменено получение настройки
		СтрокаОтчетыПараметры.Вставить("НастройкиВариантаОтчета",      Форма.Отчет.КомпоновщикНастроек.Настройки);
		СтрокаОтчетыПараметры.Вставить("НачалоПериода",
			?(ЕстьРеквизитНаКлиенте(Форма, "НачалоПериода"), Форма.НачалоПериода, Дата(1,1,1)));
		СтрокаОтчетыПараметры.Вставить("КонецПериода",
			?(ЕстьРеквизитНаКлиенте(Форма, "КонецПериода"), Форма.КонецПериода, Дата(1,1,1)));
		СтрокаОтчетыПараметры.Вставить("ВидПериода",
			?(ЕстьРеквизитНаКлиенте(Форма, "ВидПериода"), Форма.ВидПериода, ""));
		// Конец АльфаАвто
	КонецЕсли;
