...
Функция ДанныеПечатиСуммаПрописью(ОписанияИсточниковДанных, КодЯзыка)
	...
	Для Каждого ОписаниеИсточника Из ОписанияИсточниковДанных Цикл
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ОписаниеИсточника.Владелец)) Тогда
			...
			// АльфаАвто
			// Проверим через общий реквизит.
			ОбъектМетаданныхВалюты = Метаданные.ОбщиеРеквизиты.ВалютаДокумента;
			ВалютаДокумента = ОбъектМетаданныхВалюты.Состав.Найти(ОбъектМетаданных.Имя);
			Если ВалютаДокумента <> Неопределено
				И ВалютаДокумента.Использование = Метаданные.СвойстваОбъектов.ИспользованиеОбщегоРеквизита.Использовать Тогда
				Валюта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОписаниеИсточника.Владелец, ОбъектМетаданныхВалюты.Имя);
