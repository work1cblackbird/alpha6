...
Функция ВыполнитьРассылкуОтчетов(Рассылка, ПараметрыЖурнала = Неопределено, ДополнительныеНастройки = Неопределено) Экспорт
	...
	Для Каждого СтрокаОтчет Из РассылкаОбъект.Отчеты Цикл
		...
		ПолучитьПериодОтчета(Стр, СтрокаОтчет);
		// Конец АльфаАвто
		...
		// АльфаАвто
		Если СтрокаОтчет.НастройкиВариантаОтчета <> Неопределено Тогда 
			НастройкиВариантаОтчета = СтрокаОтчет.НастройкиВариантаОтчета.Получить();
			Стр.НастройкиВариантаОтчета = НастройкиВариантаОтчета;
		КонецЕсли;
		// Конец АльфаАвто
...
Функция ОтчетыДляРассылки() Экспорт
	...
	// АльфаАвто
	ТаблицаОтчетов.Колонки.Добавить("НастройкиВариантаОтчета", Новый ОписаниеТипов(МассивТиповНастроек));
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	ИначеЕсли ПараметрыОтчета.Свойство("НастройкиВариантаОтчета")
		И ТипЗнч(ПараметрыОтчета.НастройкиВариантаОтчета) = Тип("НастройкиКомпоновкиДанных") Тогда
		ПараметрыПодключения.Вставить("НастройкиКД", ПараметрыОтчета.НастройкиВариантаОтчета);
	КонецЕсли;
	// Конец АльфаАвто
	...
	Если ПараметрыОтчета.СКД Тогда
		...
		//	Найденные = ОтчетыКлиентСервер.ЭлементыНастроекОтобранные(ПользовательскиеНастройкиКД, Фильтр, , Найденные);
		...
		//		КонецЕсли;
		//	КонецЦикла;
			//НастройкиВариантаОтчета = ПараметрыОтчета.КомпоновщикНастроекКД.Настройки;
			...
			//	КонецЕсли;
			//КонецЦикла;
			Если ЗначениеЗаполнено(ПараметрыОтчета.ПолеОтбора) Тогда 
				ПараметрыОтчета.ПерсональныеОтборы.Вставить(ПараметрыОтчета.ПолеОтбора);
			КонецЕсли;
		КонецЕсли;
...
Функция СоздатьДеревоОтчетов()
	...
	// АльфаАвто
	ДеревоОтчетов.Колонки.Добавить("НастройкиВариантаОтчета", Новый ОписаниеТипов("НастройкиКомпоновкиДанных"));
	// Конец АльфаАвто
...
// АльфаАвто
Процедура ПолучитьПериодОтчета(СтрокаПриемник, СтрокаИсточник)
	
	Если СтрокаИсточник.ВидПериода = "День" Тогда
		СтрокаПриемник.НачалоПериода = НачалоДня(ТекущаяДатаСеанса());
		СтрокаПриемник.КонецПериода =  КонецДня(ТекущаяДатаСеанса());
	ИначеЕсли СтрокаИсточник.ВидПериода = "Неделя" Тогда
		СтрокаПриемник.НачалоПериода = НачалоНедели(ТекущаяДатаСеанса());
		СтрокаПриемник.КонецПериода =  КонецНедели(ТекущаяДатаСеанса());
	ИначеЕсли СтрокаИсточник.ВидПериода = "Месяц" Тогда
		СтрокаПриемник.НачалоПериода = НачалоМесяца(ТекущаяДатаСеанса());
		СтрокаПриемник.КонецПериода =  КонецМесяца(ТекущаяДатаСеанса());
	ИначеЕсли СтрокаИсточник.ВидПериода = "Квартал" Тогда
		СтрокаПриемник.НачалоПериода = НачалоКвартала(ТекущаяДатаСеанса());
		СтрокаПриемник.КонецПериода =  КонецКвартала(ТекущаяДатаСеанса());
	ИначеЕсли СтрокаИсточник.ВидПериода = "Год" Тогда
		СтрокаПриемник.НачалоПериода = НачалоГода(ТекущаяДатаСеанса());
		СтрокаПриемник.КонецПериода =  КонецГода(ТекущаяДатаСеанса());
	Иначе
		Если ТипЗнч(СтрокаИсточник.НачалоПериода) = Тип("Строка") Тогда
			ВидПериода = Неопределено;
			Попытка
				ВариантПериодаОтчета = ВариантСтандартногоПериода[СтрЗаменить(СтрокаИсточник.НачалоПериода, " ", "")];
			Исключение
			КонецПопытки;
			Если ВариантПериодаОтчета <> Неопределено Тогда
				ДатаОтчета = Новый СтандартныйПериод(ВариантПериодаОтчета);
				СтрокаПриемник.НачалоПериода = ДатаОтчета.ДатаНачала;
				СтрокаПриемник.КонецПериода = ДатаОтчета.ДатаОкончания;
			Иначе
				СтрокаПриемник.НачалоПериода =
					Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[СтрокаИсточник.НачалоПериода]);
				СтрокаПриемник.КонецПериода =
					Новый СтандартнаяДатаНачала(ВариантСтандартнойДатыНачала[СтрокаИсточник.КонецПериода]);
			КонецЕсли;
		Иначе
			СтрокаПриемник.НачалоПериода = СтрокаИсточник.НачалоПериода;
			СтрокаПриемник.КонецПериода = СтрокаИсточник.КонецПериода;
		КонецЕсли;
	КонецЕсли;
...
Функция ВыполнитьФормированиеОтчетовВОдномПотоке(Отчеты, ПараметрыДоставки, НаименованиеРассылки, ПараметрыЖурнала)
	...
	Для Каждого СтрокаОтчет Из Отчеты Цикл
		...
		// АльфаАвто
		ПараметрыОтчета = Новый Структура("Отчет, Настройки, НастройкиВариантаОтчета, Форматы, ОтправлятьЕслиПустой, ШаблонНаименования, НачалоПериода, КонецПериода");
		Если ПараметрыДоставки.Персонализирована Тогда
