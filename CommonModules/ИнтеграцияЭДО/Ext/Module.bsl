...
Функция СписокОперацийВсехТиповДокумента() Экспорт 
	...
	// АльфаАвто
	// Сформируем список оснований для исключения из списка
	СписокИсключений = Новый Массив;
	СписокИсключений.Добавить("УдалитьСоглашенияОбИспользованииЭД");
	...
	Для Каждого ТекущееОснование Из Основания Цикл
		...
		// АльфаАвто
		Если СписокИсключений.Найти(МетаданныеОснования.Имя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		// Конец АльфаАвто
...
Процедура ЗаказПоставщику_Автозаполнение(Данные, Знач ОбъектУчета)
	...
	ДополнительныеДанные.Вставить("Ид", ИдентификаторОбъектаУчета(ОбъектУчета));
	// АльфаАвто
	// Укажем номер документа в соответствии с настройкой константы выводить полный номер документа в печатных формах
	ДополнительныеДанные.Вставить("Номер", УправлениеПечатьюПлатформа.ПолучитьНомерДляПечати(ОбъектУчета));
	// Конец АльфаАвто
	ДополнительныеДанные.Вставить("Дата", Описание.Дата);
	ДополнитьДанныеДокумента(Данные, ДополнительныеДанные);
