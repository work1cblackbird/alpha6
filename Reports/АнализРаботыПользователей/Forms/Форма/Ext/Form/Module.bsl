...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РежимВариантаОтчета Тогда
		...
		Если Не КлючВариантаПоУмолчанию = Неопределено Тогда
			ЭтаФорма.КлючТекущегоВарианта = КлючВариантаПоУмолчанию;
		КонецЕсли;
	...
	Иначе
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при вызове ОпределитьНастройкиФормы'"),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при обработке структуры ограничений'"),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ОбновитьСписокПринтеровДляПечати()
	...
	ДобавитьПринтер(0, "Выбор принтера при печати");
	Счетчик = 1;
	Для Ном = 1 По Принтеры.Количество() - 1 Цикл
...
Функция ПечатьНаСервере(Знач ВременныйТабличныйДокумент)
	...
	ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_АнализРаботыПользователей";
