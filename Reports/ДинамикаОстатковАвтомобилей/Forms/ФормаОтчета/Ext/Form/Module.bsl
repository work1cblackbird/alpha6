...
//  СтандартнаяОбработка - Булево - В данный параметр передается
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//// Добавим кнопку на форму команды для ввода дополнительных параметров отчета
	//Если НЕ Элементы.Найти("НастройкаПараметровИнтервалов") = Неопределено Тогда
	//	Элементы.НастройкаПараметровИнтервалов.Видимость = ЭтотОбъект.КлючОбъекта = "Отчет.АнализПотерянныхКлиентов" И (НЕ ОтчетМетаданные.Команды.Найти("НастройкаПараметров") = Неопределено);
	//КонецЕсли;
	//Если НЕ Элементы.Найти("НастройкаПараметровСписокДокументов") = Неопределено Тогда
	//	Элементы.НастройкаПараметровСписокДокументов.Видимость = ЭтотОбъект.КлючОбъекта = "Отчет.АнализРаботыПользователей" И (НЕ ОтчетМетаданные.Команды.Найти("НастройкаПараметров") = Неопределено);
	//КонецЕсли;
...
// Обработчик события возникающего на клиенте при выборе объекта в одной из подчиненных форм
// или при вызове в одной из подчиненных форм метода.
//
// Параметры:
...
Процедура ИзмерениеСтрокиПриИзменении(Элемент)
	...
	Если Отчет.ИзмерениеСтроки = "ПериодКвартал" Тогда
		ЭлементКвартал = Элементы.ИзмерениеКолонки.СписокВыбора.НайтиПоЗначению("ПериодКвартал");
		Если ЭлементКвартал <> Неопределено Тогда
			Элементы.ИзмерениеКолонки.СписокВыбора.Удалить(ЭлементКвартал);	
		КонецЕсли;
	ИначеЕсли Элементы.ИзмерениеКолонки.СписокВыбора.Количество() = 3 Тогда 
		Элементы.ИзмерениеКолонки.СписокВыбора.Добавить("ПериодКвартал", "Период квартал");
		Элементы.ИзмерениеКолонки.СписокВыбора.Сдвинуть(3,-3);
	КонецЕсли 
	
КонецПроцедуры

#Область ОбработчикиСобытийЭлементовФормыТаблицыОтборы
...
//  								выполнения системной обработки события.
...
#Область ОбработчикиСобытийЭлементовФормыТаблицыСтроки
...
#Область ОбработчикиСобытийЭлементовФормыТаблицыКолонки
...
Функция ПечатьНаСервере(Знач ВременныйТабличныйДокумент)
	...
	ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ДинамикаОстатковАвтомобилей";
	УправлениеПечатьюПлатформа.УстановитьСтандартныеПараметрыПечати(ИмяПараметровПечати, ДокументДляПечати);
	ДокументДляПечати.ИмяПараметровПечати = ИмяПараметровПечати;
...
//
&НаСервере
Процедура ОбновитьНастройкиСКД(РежимОбновления = "Всё", ВидОтчетаДляФормыНастроек = Неопределено, ОтсекатьГруппировки = Ложь)
	...
	Если РежимОбновления = "Всё" ИЛИ РежимОбновления = "ТолькоПериод" Тогда
		// Начало периода
	...
	Если РежимОбновления = "Всё" ИЛИ РежимОбновления = "ТолькоНастройки" Тогда
...
Процедура УстановитьВидимостьЭлементов()
	...
	Если ПолучитьЗначениеПараметраСтруктуры(НастройкиОтчета.Видимость, "Параметры", Ложь) Тогда
		...
		Для Каждого ТекущийПараметр Из Настройки.ПараметрыДанных.Элементы Цикл
			...
			//// Не будем учитывать параметры, которые не выводятся
			//Если (ИмяПараметра = "СписокДокументов" И ЭтотОбъект.КлючОбъекта = "Отчет.АнализРаботыПользователей")
			//	 ИЛИ ((ИмяПараметра = "СуммовыеГруппы" ИЛИ ИмяПараметра = "КоличественныеГруппы") И ЭтотОбъект.КлючОбъекта = "Отчет.АнализПотерянныхКлиентов") Тогда
			//	Продолжить;
			//КонецЕсли;
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище(НастройкиСохранения)
	...
	РезультатПомещения = Новый Массив;
	...
	Для Каждого ТипФайла Из ВыбранныеФорматыСохранения Цикл
		...
		Исключение
			ОбщегоНазначения.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Не удалось записать файл по причине:%1'"), ОписаниеОшибки())
			);
		...
		Иначе
			...
			КонецЕсли;
			РезультатПомещения.Добавить(ОписаниеФайла);
		КонецЕсли;
	КонецЦикла;
