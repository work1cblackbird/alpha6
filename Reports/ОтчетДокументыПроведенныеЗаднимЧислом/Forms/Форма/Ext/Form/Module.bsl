...
Процедура ОбновитьНастройкиСКД(РежимОбновления = "Все", ВидОтчетаДляФормыНастроек = Неопределено, ОтсекатьГруппировки = Ложь)
	...
	// если выполняется в фоне - передаем ТЗ через настройки СКД
	МассивТиповДокументов = Новый Массив;
	МассивТиповДокументов = ОбщегоНазначения.ТаблицаЗначенийВМассив(Отчет.ТипыДокументов.Выгрузить());
	
	НастройкиСКД.ДополнительныеСвойства.Вставить("МассивТиповДокументов", МассивТиповДокументов);
...
Процедура КомпоновщикЗагрузитьНастройки(РезультатЗакрытия)
	...
	Для Каждого Строка Из РезультатЗакрытия.ТипыДокументов Цикл
		НоваяСтрока = Отчет.ТипыДокументов.Добавить();
