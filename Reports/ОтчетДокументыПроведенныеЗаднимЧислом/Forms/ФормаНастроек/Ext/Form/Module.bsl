...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ТипыДокументов") Тогда
		Отчет.ТипыДокументов.Очистить();
		Для Каждого Строка Из Параметры.ТипыДокументов Цикл
			НоваяСтрока = Отчет.ТипыДокументов.Добавить();
...
Процедура ПриЗагрузкеВариантаНаСервере(Настройки)
	...
	СтрокаПараметров = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ТипыДокументов").Значение;
	Если ЗначениеЗаполнено(СтрокаПараметров) Тогда
		
		ЗаполнитьТипыДокументов(СтрокаПараметров);
		
	КонецЕсли;
	
	Если Отчет.ТипыДокументов.Количество() = 0 Тогда
		...
		Для Каждого ВидДок из Метаданные.Документы Цикл
			...
			Если Метаданные.ОбщиеРеквизиты.ДатаОперации.Состав.Найти(ВидДок).Использование <> Использовать Тогда
				Продолжить;
			КонецЕсли;
			
			Если Метаданные.ОбщиеРеквизиты.ДатаСоздания.Состав.Найти(ВидДок).Использование <> Использовать Тогда
				Продолжить;
			КонецЕсли;
			
			Если ВидДок.Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить Тогда
				Документ = Отчет.ТипыДокументов.Добавить();
				Документ.Использовать       = Ложь;
				Документ.ТипДокумента       = ВидДок.Имя;
				Документ.Представление      = ВидДок.Синоним;
				Документ.ЕстьСуммаДокумента = Метаданные.ОбщиеРеквизиты.СуммаДокумента.Состав.Найти(ВидДок).Использование = Использовать;
				Документ.ЕстьХозОперация    = Метаданные.ОбщиеРеквизиты.ХозОперация.Состав.Найти(ВидДок).Использование = Использовать;
			КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТипыДокументов(Строка)
	
	Отчет.ТипыДокументов.Очистить();
	МассивДокументов = ЗначениеИзСтрокиВнутр(Строка);
...
&НаСервере
Процедура ВыбратьВсеНаСервере(Использовать=Неопределено)
	Для Каждого Строка Из Отчет.ТипыДокументов Цикл
		Строка.Использовать = ?(Использовать=Неопределено, НЕ Строка.Использовать, Использовать);
	КонецЦикла;
КонецПроцедуры
...
&НаКлиенте
Процедура ОбратитьВыделение(Команда)
	ВыбратьВсеНаСервере();
КонецПроцедуры
...
Процедура СформироватьОтчет(Команда)
	...
	ВыделенныеЭлементы = Ложь;
	Для Каждого Строка Из Отчет.ТипыДокументов Цикл
		Если Строка.Использовать Тогда
			ВыделенныеЭлементы = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если НЕ ВыделенныеЭлементы Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Необходимо выбрать тип документа'"));
		Возврат;
	КонецЕсли;
	...
	Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра("ТипыДокументов", ВыгрузитьТипыДокументовВСтроку());
	СтруктураПараметров = Новый Структура;
	...
	СтруктураПараметров.Вставить("ТипыДокументов",        ВыгрузитьТипыДокументов());
	Закрыть(СтруктураПараметров);
	
КонецПроцедуры //СформироватьОтчет()

&НаСервере
Функция ВыгрузитьТипыДокументовВСтроку()
	Возврат ЗначениеВСтрокуВнутр(ВыгрузитьТипыДокументов());
КонецФункции

&НаСервере
Функция ВыгрузитьТипыДокументов()
	Возврат ОбщегоНазначения.ТаблицаЗначенийВМассив(Отчет.ТипыДокументов.Выгрузить());
КонецФункции
