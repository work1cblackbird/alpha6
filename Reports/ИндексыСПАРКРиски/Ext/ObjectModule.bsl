...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ИменаИсключаемыхОтборов = Новый Массив;
	ИменаИсключаемыхОтборов.Добавить("ВидПериода");
	ИменаИсключаемыхОтборов.Добавить("КонецПериода");
	ИменаИсключаемыхОтборов.Добавить("НачалоПериода");
	Для Каждого ТекущийОтбор Из НастройкиОтчета.Отбор.Элементы Цикл
		Если ТипЗнч(ТекущийОтбор) = Тип("ЭлементОтбораКомпоновкиДанных")
			И ИменаИсключаемыхОтборов.Найти(Строка(ТекущийОтбор.ЛевоеЗначение)) <> Неопределено Тогда
			УдаляемыеОтборы.Добавить(ТекущийОтбор);
		КонецЕсли;
	КонецЦикла;
	Для Каждого УдаляемыйОтбор Из УдаляемыеОтборы Цикл
		НастройкиОтчета.Отбор.Элементы.Удалить(УдаляемыйОтбор);
	КонецЦикла;
	// Конец АльфаАвто
