...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	МассивВидовРесурсов.Добавить("Сотрудник");
	МассивВидовРесурсов.Добавить("Цех");
	
	КлючВарианта = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ВариантОтчета");
	Если КлючВарианта <> Неопределено Тогда
		КлючВарианта = КлючВарианта.Значение;
	КонецЕсли;
	
	Если КлючВарианта = "ПоИсполнителям" Тогда
		КлючВарианта = "Сотрудник";	
	ИначеЕсли КлючВарианта = "ПоЦехам" Тогда
		КлючВарианта = "Цех";
	КонецЕсли;
	
	Отчеты.ДоступностьРесурсовАвтосервиса.УстановитьОграничениеПолей(КлючВарианта, СхемаКомпоновки, МассивВидовРесурсов);
...
Процедура ПараметрыЗначениеПриИзменении(Элемент)
	...
	Отбор.ПолучитьЭлементы().Очистить();
	...
	Если РежимОбновления = "Все" ИЛИ РежимОбновления = "ТолькоНастройки" Тогда
		...
		Если НЕ ГруппировкиЗапрещены Тогда
			...
			Если ВидОтчетаДляФормыНастроек = 0 Тогда
				...
				// mikolv +
				КлючВарианта = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ВариантОтчета");
				Если КлючВарианта <> Неопределено И ГруппировкиСтрок.Количество()<>0 Тогда
					КлючВарианта = КлючВарианта.Значение;
					...
					ПолеКомпановки = Новый ПолеКомпоновкиДанных(КлючВарианта);
					
					Индекс = -1;
					ИндексСтроки = -1;
					Для каждого Строка Из ГруппировкиСтрок Цикл
						Индекс = Индекс + 1;
						Если Строка.Заголовок = "Детальные записи" И Строка.ПоляГруппировки.Количество() = 0 Тогда
							ИндексСтроки = Индекс;	
						КонецЕсли;
						Если Строка.Заголовок = "Цех" ИЛИ Строка.Заголовок = "Сотрудник" Тогда
							ИндексСтроки = -1;
							Прервать;
						КонецЕсли;
					КонецЦикла; 
					...
					Если ИндексСтроки >=0 Тогда
						Поле = ГруппировкиСтрок[ИндексСтроки].ПоляГруппировки.Добавить();
						Поле.Поле = ПолеКомпановки; 
