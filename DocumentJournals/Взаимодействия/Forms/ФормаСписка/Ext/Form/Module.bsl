...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	ВзаимодействияАльфаАвто.ЖурналВзаимодействияПриСозданииНаСервере(ЭтотОбъект, Истина);
	// Конец АльфаАвто
...
Процедура ЗаполнитьДеревоСвойств(ИмяКоманды = "")
	...
	Если ТекущееСвойствоПанелиНавигацииЯвляетсяРеквизитом Тогда
		// АльфаАвто
		// Удалены неиспользуемые документы
	...
	Иначе
		...
		|	ГДЕ
		|		НЕ ДокументВзаимодействий.Ссылка ССЫЛКА Документ.Встреча
		|		И НЕ ДокументВзаимодействий.Ссылка ССЫЛКА Документ.ЗапланированноеВзаимодействие) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.Значение
		|
...
Процедура ЗаполнитьПанельПредметов()
	...
	Если ТолькоЗначимыеПредметы Тогда
		...
		ТекстЗапросаДинамическийСписок = 
			...
			|	И НЕ СостоянияПредметовВзаимодействий.Предмет ССЫЛКА Документ.Встреча
			|	И НЕ СостоянияПредметовВзаимодействий.Предмет ССЫЛКА Документ.ЗапланированноеВзаимодействие";
		// Конец АльфаАвто
...
Процедура ЗаполнитьТаблицуКатегорий()
	...
	// АльфаАвто
	// Убраны неиспользуемые документы
	Запрос.Текст = Запрос.Текст + "
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
