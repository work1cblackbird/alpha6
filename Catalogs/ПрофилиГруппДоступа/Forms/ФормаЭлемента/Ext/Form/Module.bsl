...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	УправлениеДоступомАльфаАвто.ФормаПрофилиГруппДоступаПриСозданииНаСервере(ЭтотОбъект);
	// Конец АльфаАвто
...
&НаСервере
Функция СохранитьНастройкиПрофиляВФайлНаСервере(Адрес)
	...
	ДопустимыеСимволы = ДопустимыеСимволы + "abcdefghijklmnopqrstuvwxyzабвгдеёжзийклмнопрстуфхцчшщъыьэюя";
	НачалоЭлемента = ОбщегоНазначенияАвтосалонКлиентСервер.ОтфильтроватьСтроку(НачалоЭлемента, ДопустимыеСимволы);
	
	Если ПустаяСтрока(НачалоЭлемента) ИЛИ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(НачалоЭлемента) Тогда
		НачалоЭлемента = НСтр("ru='ПрофильГруппыДоступа'") + НачалоЭлемента;
	КонецЕсли;
	...
	АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(Файл, Адрес);
...
&НаКлиенте
Процедура Подключаемый_СохранитьНастройкиПрофиляВФайл(Команда)
	...
	Диалог.Показать(
		Новый ОписаниеОповещения("СохранитьНастройкиПрофиляВФайлЗавершение", ЭтотОбъект, Новый Структура("Диалог", Диалог)));
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьНастройкиПрофиляВФайлЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	
	Диалог = ДополнительныеПараметры.Диалог;
	
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		ПолноеИмяФайла = Диалог.ПолноеИмяФайла;
		Адрес = СохранитьНастройкиПрофиляВФайлНаСервере(УникальныйИдентификатор);
		НачатьПолучениеФайлаССервера(, Адрес, ПолноеИмяФайла);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ЗагрузитьНастройкиПрофиляИзФайла(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ЗагрузитьНастройкиПрофиляИзФайлаЗавершение", ЭтотОбъект,
...
&НаКлиенте
Процедура ЗагрузитьНастройкиПрофиляИзФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда
		...
		ЗагрузитьНастройкиПрофиляИзФайлаНаСервере(Адрес);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьНастройкиПрофиляИзФайлаНаСервере(Адрес)
		...
		Если СтруктураПрофиля.Свойство("Роли") И СтруктураПрофиля.Роли <> Неопределено Тогда
			Для Каждого Строка Из СтруктураПрофиля.Роли Цикл
				...
				Если Роль <> Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
					Для Каждого СтрокаРоль Из Роли.ПолучитьЭлементы() Цикл
					...
					КонецЦикла;
				Иначе
					ТекстСообщения = НСтр("ru = 'Роль с полным наименованием'") + " " 
						+ Строка.ПолноеИмя + " " + НСтр("ru = 'не найдена'");
					ВывестиСообщение(ТекстСообщения);
					Продолжить;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		Если СтруктураПрофиля.Свойство("ВидыДоступа")
			И СтруктураПрофиля.ВидыДоступа <> Неопределено
			И СтруктураПрофиля.ВидыДоступа.Количество() > 0 Тогда
			Объект.ВидыДоступа.Загрузить(СтруктураПрофиля.ВидыДоступа.Выгрузить());
		КонецЕсли;
		Если СтруктураПрофиля.Свойство("ЗначенияДоступа")
			И СтруктураПрофиля.ЗначенияДоступа <> Неопределено
			И СтруктураПрофиля.ЗначенияДоступа.Количество() > 0 Тогда
