...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если Владелец.Пустая() Тогда
		Владелец = Справочники.Контрагенты.ОсновнойПоставщик;
	КонецЕсли;
	
	Если ВидПрайсЛиста.Пустая() Тогда
		ВидПрайсЛиста = Перечисления.ВидыПрайсЛистов.ПрайсЛистПоставщика;
	КонецЕсли;
	
	Если ПустаяСтрока(Наименование) Тогда
		Наименование = ПрайсЛистыКонтрагентовКлиентСервер.СформироватьНаименованиеПрайсЛиста(ЭтотОбъект);
	КонецЕсли;
	
	Если КоэффициентПересчетаВеса = 0 Тогда
		КоэффициентПересчетаВеса  = 1000;
	КонецЕсли;
	
	Если ЭтоНовый() Тогда
		Валюта                = Константы.ВалютаРегламентированногоУчетаОрганизаций.Получить();
		ХранитьДанныеЛокально = Истина;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ Справочники.ПрайсЛистыКонтрагентов.ПроверитьКорректностьСтавкиНДС(ЭтотОбъект) Тогда
