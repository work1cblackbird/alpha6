// Модуль формы списка справочника "Интерфейсы для внешних систем"
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Тогда
		ОценкаПроизводительностиКлиент.ЗамерВремени("КопированиеСправочникаЦеновыеГруппы");
	Иначе
		ОценкаПроизводительностиКлиент.ЗамерВремени("СозданиеФормыСправочникаЦеновыеГруппы");
	КонецЕсли;
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	// ОценкаПроизводительности
	ОценкаПроизводительностиКлиент.ЗамерВремени("ОткрытиеФормыСправочникаЦеновыеГруппы");
	// Конец ОценкаПроизводительности
...
Процедура ДеревоПриАктивизацииСтроки(Элемент)
	...
	Если Элементы.Список.Видимость Тогда
		...
		ЗначенияРавны = (Элементы.Дерево.ТекущаяСтрока = Элементы.Список.ТекущийРодитель);
		
		Если НЕ ЗначенияРавны Тогда
			ЗначенияРавны = (НЕ ЗначениеЗаполнено(Элементы.Список.ТекущийРодитель)
...
Функция ПолучитьСписокУчастниковСБПНаСервере()
	
	РезультатВыполнения = ОнлайнОплата.ПолучитьСтруктуруРезультатВыполнения();
	ПлатежныйШлюзСБП_1С.ОбновитьНастройкиУчастников(РезультатВыполнения);
...
&НаКлиенте
Процедура ПолучитьСписокУчастниковСБП(Команда)
	
	РезультатВыполнения = ПолучитьСписокУчастниковСБПНаСервере();
	Если РезультатВыполнения.Успешно Тогда
		
		ТекстПредупреждения = НСтр("ru='Список участников СБП загружен'");
