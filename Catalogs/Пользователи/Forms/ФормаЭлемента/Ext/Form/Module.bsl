...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// АльфаАвто
	ПользователиАльфаАвто.ФормаПользователиПриСозданииНаСервере(ЭтотОбъект);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	// АльфаАвто
	Если (НЕ ЭтотОбъект.ПропуститьПроверкуПодразделения) И ЭтотОбъект.ПользовательСотрудник Тогда
		Если ЗначениеЗаполнено(ЭтотОбъект.СотрудникОбъект.Ссылка) Тогда
			ПодразделениеСотрудника = ПодразделениеСотрудникаПользователя(ЭтотОбъект.СотрудникОбъект.Ссылка);
		Иначе
			ПодразделениеСотрудника = ЭтотОбъект.СотрудникОбъект.ПодразделениеКомпании;
		КонецЕсли;
		Если ПодразделениеСотрудника <> Объект.ПодразделениеКомпании Тогда
			Отказ = Истина;
			ТекстВопроса = СтрШаблон(
				НСтр("ru = 'Изменить подразделение сотрудника с %1 на %2 в соответствии с параметрами пользователя?'"),
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ОбновитьИнформациюОСотруднике(Отказ, ТекущийОбъект);
	// Конец АльфаАвто
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ПродолжатьЗаполнение = УправлениеДиалогомСправочникаКлиент.ПередЗакрытием(
		ЭтотОбъект,
		Отказ,
		ЗавершениеРаботы,
		ТекстПредупреждения,
...
//
&НаКлиенте
Процедура Подключаемый_ПодразделениеКомпанииПриИзменении(Элемент)
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Пользователь сотрудник".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
...
Процедура ПользовательСотрудникПриИзмененииНаСервере()
	...
	Если (НЕ ЭтотОбъект.ПользовательСотрудник) И ЗначениеЗаполнено(Объект.Сотрудник) Тогда
		Объект.Сотрудник = ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка");
		СотрудникПриИзмененииНаСервере();
	КонецЕсли;
	
	Элементы.Сотрудник.Доступность          = ЭтотОбъект.ПользовательСотрудник;
	Элементы.СотрудникДолжность.Доступность = ЭтотОбъект.ПользовательСотрудник;
	Элементы.ГруппаДаты.Доступность         = ЭтотОбъект.ПользовательСотрудник;
	Элементы.СотрудникПраво.Доступность     = ЭтотОбъект.ПользовательСотрудник;
	
КонецПроцедуры // ПользовательСотрудникПриИзмененииНаСервере()
...
//
&НаКлиенте
Процедура Подключаемый_ПользовательСотрудникПриИзменении(Элемент)
	
	ПользовательСотрудникПриИзмененииНаСервере();
	
КонецПроцедуры // ПользовательСотрудникПриИзменении()
...
Процедура СотрудникПриИзмененииНаСервере()
	...
	Если ПользовательСотрудник Тогда
		...
		АдресФотографии = ПоместитьВоВременноеХранилище(Справочники.Сотрудники.ПолучитьФотографию(Объект.Сотрудник), УникальныйИдентификатор);
	ИначеЕсли ПравоДоступа("Добавление", Метаданные.Справочники.Сотрудники) Тогда
		ЗначениеВДанныеФормы(Справочники.Сотрудники.СоздатьЭлемент(), ЭтотОбъект.СотрудникОбъект);
		АдресФотографии = "";
	КонецЕсли;
	Элементы.СотрудникДатаУвольнения.Доступность = ЭтотОбъект.СотрудникОбъект.ФлагУволен;
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		Объект.Организация = ЭтотОбъект.СотрудникОбъект.Организация;
		ОрганизацияПриИзмененииНаСервере();
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ПодразделениеКомпании) Тогда
		Объект.ПодразделениеКомпании = ЭтотОбъект.СотрудникОбъект.ПодразделениеКомпании;
		ПодразделениеКомпанииПриИзмененииНаСервере();
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Наименование) Тогда
		Объект.Наименование = ЭтотОбъект.СотрудникОбъект.Наименование;
		ОбновитьИмяДляВхода(ЭтотОбъект, Истина);
		УстановитьНеобходимостьСинхронизацииССервисом(ЭтотОбъект);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.ГрафикРаботы) Тогда
		Объект.ГрафикРаботы = ЭтотОбъект.СотрудникОбъект.ГрафикРаботы;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ЭтотОбъект[ТелефонПользователя.ИмяРеквизита]) Тогда
		Телефон = ЭтотОбъект.СотрудникОбъект.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.ТелефонСотрудника));
	...
	Если НЕ ЗначениеЗаполнено(ЭтотОбъект[EmailПользователя.ИмяРеквизита]) Тогда
		Email = ЭтотОбъект.СотрудникОбъект.КонтактнаяИнформация.НайтиСтроки(Новый Структура("Вид", Справочники.ВидыКонтактнойИнформации.EmailСотрудника));
...
//
&НаКлиенте
Процедура Подключаемый_СотрудникПриИзменении(Элемент)
...
// Обработчик события возникающего на клиенте при изменении данных реквизита "Сотрудник дата увольнения".
//
// Параметры:
//  Элемент - ПолеФормы - Элемент управления, в котором возникло данное событие.
//
&НаКлиенте
Процедура Подключаемый_СотрудникУволенПриИзменении(Элемент)
	
	Если ЭтотОбъект.СотрудникОбъект.ФлагУволен И НЕ ЗначениеЗаполнено(ЭтотОбъект.СотрудникОбъект.ДатаУвольнения) Тогда
		ЭтотОбъект.СотрудникОбъект.ДатаУвольнения = ОбщегоНазначенияКлиент.ДатаСеанса();
	КонецЕсли;
	Элементы.СотрудникДатаУвольнения.Доступность = ЭтотОбъект.СотрудникОбъект.ФлагУволен;
	
КонецПроцедуры // СотрудникУволенПриИзменении()
...
&НаСервере
Процедура ОбновитьИнформациюОСотруднике(Отказ, ТекущийОбъект)
	
	Если НЕ ЭтотОбъект.ПользовательСотрудник Тогда
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.СотрудникОбъект.Ссылка) Тогда
		...
		Для каждого Поле Из СтрукутраПроверкиРеквизитов Цикл
			Если Объект.Сотрудник[Поле.Ключ] <> ЭтотОбъект.СотрудникОбъект[Поле.Ключ] Тогда
	...
	Иначе
		МодифицированностьСотрудника = ИСТИНА;
		ЭтотОбъект.СотрудникОбъект.ПодразделениеКомпании = Объект.ПодразделениеКомпании;
		ЭтотОбъект.СотрудникОбъект.Организация           = Объект.Организация;
		ЭтотОбъект.СотрудникОбъект.Наименование          = Объект.Наименование;
	КонецЕсли;
	
	Если МодифицированностьСотрудника Тогда
		
		Если ЗначениеЗаполнено(ЭтотОбъект.СотрудникОбъект.Ссылка) Тогда
			СотрудникОбъектДляЗаписи = ЭтотОбъект.СотрудникОбъект.Ссылка.ПолучитьОбъект();
			ЗаполнитьЗначенияСвойств(
				СотрудникОбъектДляЗаписи,
				ЭтотОбъект.СотрудникОбъект,
				"Должность,ДатаРождения,ДатаПриема,ФлагУволен,ДатаУвольнения");
			Если ЭтотОбъект.СотрудникОбъект.ПодразделениеКомпании = ТекущийОбъект.ПодразделениеКомпании Тогда
		...
		Попытка
			...
			КонецЕсли;
			ЗначениеВРеквизитФормы(СотрудникОбъектДляЗаписи, "СотрудникОбъект");
		Исключение
			Отказ = ИСТИНА;
		КонецПопытки;
	КонецЕсли;
...
Процедура ПослеВопросаОСменеПодразделенияСотрудника(РезультатОповещения, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РезультатОповещения = КодВозвратаДиалога.Да Тогда
		ЭтотОбъект.СотрудникОбъект.ПодразделениеКомпании = Объект.ПодразделениеКомпании;
		ЭтотОбъект.СотрудникОбъект.Организация           = Объект.Организация;
	ИначеЕсли ЗначениеЗаполнено(ЭтотОбъект.СотрудникОбъект.Ссылка) Тогда
		// Обновим из базы актуальные данные по сотруднику.
		ЗаполнитьЗначенияСвойств(ЭтотОбъект.СотрудникОбъект, ПолучитьДанныеСотрудника(ЭтотОбъект.СотрудникОбъект.Ссылка));
	КонецЕсли;
...
КонецПроцедуры // ПослеВопросаОСменеПодразделенияСотрудника()
