...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.ВнесеныИзменения Тогда
			...
			КонецЕсли;
			Если ЭтоАдресВременногоХранилища(СтрокаОтчеты.АдресНастройкиВариантаОтчета) Тогда
				НастройкиВариантаОтчета = ПолучитьИзВременногоХранилища(СтрокаОтчеты.АдресНастройкиВариантаОтчета);
				ОбъектСтрокаОтчеты.НастройкиВариантаОтчета = Новый ХранилищеЗначения(НастройкиВариантаОтчета, Новый СжатиеДанных(9));
			КонецЕсли;
			// Конец АльфаАвто
		...
		Иначе
			...
			Если ЭтоАдресВременногоХранилища(СтрокаОтчеты.АдресНастройкиВариантаОтчета) Тогда
				НастройкиВариантаОтчета = ПолучитьИзВременногоХранилища(СтрокаОтчеты.АдресНастройкиВариантаОтчета);
			Иначе
				НастройкиВариантаОтчета = ОбъектСтрокаОтчеты.НастройкиВариантаОтчета.Получить();
			КонецЕсли;
			// Конец АльфаАвто
		...
		// АльфаАвто
		ПараметрыОтчета = ИнициализироватьОтчет(СтрокаОтчеты, Истина, ПользовательскиеНастройки, НастройкиВариантаОтчета, Ложь);
		// Конец АльфаАвто
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			//КонецЕсли;
			Если ПараметрыОтчета.Свойство("ПолеОтбора") Тогда 
				ПолеОтбора = ПараметрыОтчета.ПолеОтбора;
			Иначе 
				ПолеОтбора = Неопределено;
			КонецЕсли;
			Если ЗначениеЗаполнено(ПолеОтбора) Тогда 
			    Для Каждого ОтборКД Из НастройкиВариантаОтчета.Отбор.Элементы Цикл
					Если ТипЗнч(ОтборКД) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
						Если Строка(ОтборКД.ЛевоеЗначение) = ПолеОтбора Тогда 
							//ОтборКД.ПравоеЗначение = Получатель;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Элементы.ПолеОтбора.Видимость = Элементы.ПолеОтбора.Доступность;
	// Конец АльфаАвто
КонецПроцедуры
...
Процедура ПользовательскиеНастройкиПриИзменении(Элемент)
	...
	СтруктураПараметров.Вставить("КлючВарианта",          СокрЛП(СтрокаОтчеты.КлючВарианта));
	СтруктураПараметров.Вставить("ПредставлениеВарианта", СокрЛП(СтрокаОтчеты.Представление));
	СтруктураПараметров.Вставить("Вариант",               ОтчетыПлатформаВызовСервера.КомпоновщикПолучитьНастройки(КомпоновщикНастроекКД));

	Подключаемый_ОповещениеИзмененияНастроек(СтруктураПараметров, Неопределено);
	// Конец АльфаАвто
...
Процедура ОткрытьНастройкиОтчета(Команда)
	...
	СтруктураПараметров.Вставить("НачалоПериода",             НачалоДня(ЭтотОбъект.ДатаОтчета.ДатаНачала));
	СтруктураПараметров.Вставить("КонецПериода",              КонецДня(ЭтотОбъект.ДатаОтчета.ДатаОкончания));
	СтруктураПараметров.Вставить("ВидПериода",                ЭтотОбъект.ВидПериода);
	...
	Если ЭтотОбъект.РежимЭксперт Тогда
		ОткрытьФорму(СтрокаОтчеты.ПолноеИмя+".ФормаВарианта", СтруктураПараметров, ЭтотОбъект,,,, ОписаниеОповещенияИзмененияНастроек, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		СтруктураПараметров.Вставить("ОтсекатьОтключенныеГруппировки", Истина);
		ОткрытьФорму(СтрокаОтчеты.ПолноеИмя+".ФормаНастроек", СтруктураПараметров, ЭтотОбъект,,,, ОписаниеОповещенияИзмененияНастроек, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура ПериодПроизвольный(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ЭтотОбъект["ДатаОтчета"].ДатаНачала) Тогда 
		Если ЗначениеЗаполнено(ЭтотОбъект["ДатаОтчета"].ДатаОкончания) Тогда 
			ЭтотОбъект["ДатаОтчета"].ДатаНачала = ЭтотОбъект["ДатаОтчета"].ДатаОкончания;
		Иначе 
			ЭтотОбъект["ДатаОтчета"].ДатаНачала = ТекущаяДата();
		КонецЕсли;
	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(ЭтотОбъект["ДатаОтчета"].ДатаОкончания) Тогда 
		ЭтотОбъект["ДатаОтчета"].ДатаОкончания = ЭтотОбъект["ДатаОтчета"].ДатаНачала;
	КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьВидПериода(Команда)

	НачалоПериода = ТекущаяДата();
	КонецПериода = НачалоПериода;
	ОтчетыПлатформаКлиентСервер.ОбновитьВариантыПериодов(ЭтотОбъект);
	ЭтотОбъект.ДатаОтчета.ДатаНачала = НачалоПериода;
	ЭтотОбъект.ДатаОтчета.ДатаОкончания = КонецПериода;
	ОтчетыПлатформаКлиентСервер.УстановитьПометкуНаПериод(ЭтотОбъект, Команда.Имя);
...
Процедура ВидимостьПериода()
	...
	Произвольный = (ЭтотОбъект.ВидПериода = "Произвольный");
	
	Если (НЕ ЭтотОбъект.ЕстьКонецПериода) И (НЕ ЭтотОбъект.ЕстьНачалоПериода) Тогда
	...
	Иначе
		...
		Если ЭтотОбъект.ЕстьНачалоПериода Тогда
			Элементы.ОсновныеДействияПериод.Заголовок 	= "Период с";
			Элементы.ДатаОтчета.ПоложениеЗаголовка 		= ПоложениеЗаголовкаЭлементаФормы.Нет;
		Иначе
			Элементы.ОсновныеДействияПериод.Заголовок 	= "Период";
			Элементы.ДатаОтчета.Заголовок 				= "На дату";
			Элементы.ДатаОтчета.ПоложениеЗаголовка 		= ПоложениеЗаголовкаЭлементаФормы.Авто;
		КонецЕсли;
		...
		Элементы.ДатаОтчета.Видимость  = (ЭтотОбъект.ЕстьНачалоПериода ИЛИ ЭтотОбъект.ЕстьКонецПериода) И (Произвольный);
	КонецЕсли;
	
	ПользовательскиеНастройкиДоступныДляИзменения = (КомпоновщикНастроекКД.ПользовательскиеНастройки.Элементы.Количество() > 0);
	
	Элементы.ПустаяСтраница.Видимость                  = ПользовательскиеНастройкиДоступныДляИзменения;
	Элементы.ПользовательскиеНастройки.Видимость       = ПользовательскиеНастройкиДоступныДляИзменения;
	Элементы.СтраницаНастройкиТекущегоОтчета.Видимость = ПользовательскиеНастройкиДоступныДляИзменения;
	
	// АльфаАвто
	Элементы.ОткрытьНастройкиОтчета.Видимость          = НЕ ПользовательскиеНастройкиДоступныДляИзменения;
	Элементы.СброситьНастройкиОтчета.Видимость		   = НЕ ПользовательскиеНастройкиДоступныДляИзменения;
	Элементы.ПолеОтбора.Доступность					   = (НЕ ПользовательскиеНастройкиДоступныДляИзменения) И Объект.Персонализирована;
	Элементы.ПолеОтбора.Видимость                      = (НЕ ПользовательскиеНастройкиДоступныДляИзменения) И Объект.Персонализирована;
	// Конец АльфаАвто
...
Процедура ДатаОтчетаПриИзменении(Элемент)
	...
	ЭтотОбъект["ДатаОтчета"].Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод;
...
Процедура Подключаемый_ОповещениеИзмененияНастроек(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия.Свойство("НачалоПериода") Тогда 
		РезультатЗакрытия.Свойство("НачалоПериода", ЭтотОбъект.ДатаОтчета.ДатаНачала);
	КонецЕсли;
	Если РезультатЗакрытия.Свойство("КонецПериода") Тогда 
		РезультатЗакрытия.Свойство("КонецПериода",  ЭтотОбъект.ДатаОтчета.ДатаОкончания);
	КонецЕсли;
	Если РезультатЗакрытия.Свойство("ВидПериода") Тогда 
		РезультатЗакрытия.Свойство("ВидПериода",    ЭтотОбъект.ВидПериода);
	КонецЕсли;
	...
	Если НЕ СтрокаОтчеты.КлючВарианта = РезультатЗакрытия.КлючВарианта Тогда
		...
		ВариантСсылка = ПолучитьСсылкуВариантОтчета(СтрокаОтчеты.ПолноеИмя, РезультатЗакрытия.КлючВарианта);
		
		// АльфаАвто
		СтрокаОтчеты.КлючВарианта  = РезультатЗакрытия.КлючВарианта;
		СтрокаОтчеты.Отчет         = ВариантСсылка;
		СтрокаОтчеты.Представление = РезультатЗакрытия.ПредставлениеВарианта;
		// Конец АльфаАвто
	...
	Для Каждого ТекНастройка Из ПользовательскиеНастройки.Элементы Цикл
		Если ЕстьРеквизитНаКлиенте(ТекНастройка, "Параметр") И ТекНастройка.Параметр = Новый ПараметрКомпоновкиДанных("Период") Тогда
			ЭтотОбъект.ВидПериода = ?(ТекНастройка.Значение.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод, "Произвольный", "");
			ЭтотОбъект.ДатаОтчета.ДатаНачала = ТекНастройка.Значение.ДатаНачала;
			ЭтотОбъект.ДатаОтчета.ДатаОкончания = ТекНастройка.Значение.ДатаОкончания;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	// АльфаАвто
	Если РезультатЗакрытия.Свойство("НачалоПериода")
		ИЛИ РезультатЗакрытия.Свойство("КонецПериода")
		ИЛИ РезультатЗакрытия.Свойство("ВидПериода") Тогда 

		ЭтотОбъект.ЕстьНачалоПериода = ЗначениеЗаполнено(ЭтотОбъект.ДатаОтчета.ДатаНачала);
		ЭтотОбъект.ЕстьКонецПериода  = ЗначениеЗаполнено(ЭтотОбъект.ДатаОтчета.ДатаОкончания);

	Иначе 

		ЭтотОбъект.ЕстьНачалоПериода = Ложь;
		ЭтотОбъект.ЕстьКонецПериода  = Ложь;
	...
	ОтчетыПлатформаКлиентСервер.УстановитьПометкуНаПериод(ЭтотОбъект, "Период"+ЭтотОбъект.ВидПериода);
	ВидимостьПериода();
...
Функция ПолучитьСсылкуВариантОтчета(ОтчетПолноеИмя, КлючВарианта)
	...
	ОтчетИнформация = ВариантыОтчетов.ИнформацияОбОтчете(ОтчетПолноеИмя);
	ВариантСсылка	= ВариантыОтчетов.ВариантОтчета(ОтчетИнформация.Отчет, КлючВарианта);
...
&НаСервере
Процедура ДобавлениеРеквизитовПериодов()
	
	ДобавляемыеРеквизиты = Новый Массив; 
	НовыйРеквизит        = Новый РеквизитФормы("ДатаОтчета", 				Новый ОписаниеТипов("СтандартныйПериод"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("НачалоПериода", 			Новый ОписаниеТипов("Дата"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("КонецПериода", 				Новый ОписаниеТипов("Дата"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("ЕстьКонецПериода", 			Новый ОписаниеТипов("Булево"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("ЕстьНачалоПериода", 		Новый ОписаниеТипов("Булево"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("РежимЭксперт", 				Новый ОписаниеТипов("Булево"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("ВидПериода", 				Новый ОписаниеТипов("Строка"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("ВариантПериода", 			Новый ОписаниеТипов("Дата"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("ПолеОтбора", 				Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(100)));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	НовыйРеквизит        = Новый РеквизитФормы("СписокВыбораПоляОтбора", 	Новый ОписаниеТипов("СписокЗначений"));
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	
	ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	Элементы.СтраницаКомпоновщик.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	// Добавление групп
	ОсновныеДействия 						= Элементы.Вставить("ОсновныеДействия", 				Тип("ГруппаФормы"), Элементы.СтраницаКомпоновщик, Элементы.ПользовательскиеНастройки);
	ОсновныеДействия.Вид            		= ВидГруппыФормы.ОбычнаяГруппа;
	//
	ОсновныеДействияКнопки 							= Элементы.Вставить("ОсновныеДействияКнопки", 	Тип("ГруппаФормы"), ОсновныеДействия);
	ОсновныеДействияКнопки.Вид      				= ВидГруппыФормы.КоманднаяПанель;
	ОсновныеДействияКнопки.РастягиватьПоГоризонтали = Ложь;
	ОсновныеДействияКнопки.Ширина 					= 0;
	//
	ОсновныеДействияПодменюПериод 			= Элементы.Вставить("ОсновныеДействияПодменюПериод",	Тип("ГруппаФормы"), ОсновныеДействияКнопки);
	ОсновныеДействияПодменюПериод.Вид 		= ВидГруппыФормы.Подменю;
	ОсновныеДействияПодменюПериод.Заголовок = "Период";
	// Создание команд
	НоваяКоманда 			= Команды.Добавить("ОткрытьНастройкиОтчета");     
	НоваяКоманда.Действие 	= "ОткрытьНастройкиОтчета";
	//
	НоваяКоманда 			= Команды.Добавить("СброситьНастройкиОтчета");     
	НоваяКоманда.Действие 	= "СброситьНастройкиОтчета";
	НоваяКоманда.Подсказка	= "Сбросить настройки отчета";
	//
	НоваяКоманда 			= Команды.Добавить("ПериодПроизвольный");
	НоваяКоманда.Заголовок 	= "Период с";
	НоваяКоманда.Действие 	= "ПериодПроизвольный";
	//
	НоваяКоманда 			= Команды.Добавить("ПериодДень");
	НоваяКоманда.Заголовок 	= "Период день";
	НоваяКоманда.Действие 	= "ПериодДень";
	//
	НоваяКоманда 			= Команды.Добавить("ПериодНеделя");
	НоваяКоманда.Заголовок 	= "Период неделя";
	НоваяКоманда.Действие 	= "ПериодНеделя";
	//
	НоваяКоманда 			= Команды.Добавить("ПериодМесяц");
	НоваяКоманда.Заголовок 	= "Период месяц";
	НоваяКоманда.Действие 	= "ПериодМесяц";
	//
	НоваяКоманда 			= Команды.Добавить("ПериодКвартал");
	НоваяКоманда.Заголовок 	= "Период квартал";
	НоваяКоманда.Действие 	= "ПериодКвартал";
	//
	НоваяКоманда 			= Команды.Добавить("ПериодГод");
	НоваяКоманда.Заголовок 	= "Период год";
	НоваяКоманда.Действие 	= "ПериодГод";
	// Добавление команд на форму
	НовыйЭлемент 			= Элементы.Вставить("ОткрытьНастройкиОтчета", Тип("КнопкаФормы"), ОсновныеДействияКнопки, ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "ОткрытьНастройкиОтчета";
	НовыйЭлемент.Заголовок  = "Настройки...";
	НовыйЭлемент.Картинка   = БиблиотекаКартинок.НастройкиОтчета;
	НовыйЭлемент.Отображение= ОтображениеКнопки.КартинкаИТекст;
	//
	НовыйЭлемент 			= Элементы.Вставить("СброситьНастройкиОтчета", Тип("КнопкаФормы"), ОсновныеДействияКнопки, ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "СброситьНастройкиОтчета";
	НовыйЭлемент.Заголовок  = "Сброс настроек";
	НовыйЭлемент.Картинка   = БиблиотекаКартинок.СтандартнаяНастройкаКомпоновкиДанных;
	НовыйЭлемент.Отображение= ОтображениеКнопки.КартинкаИТекст;
	//
	НовыйЭлемент 			= Элементы.Добавить("ОсновныеДействияПериод", Тип("КнопкаФормы"), ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "ПериодПроизвольный";
	//
	НовыйЭлемент 			= Элементы.Добавить("ОсновныеДействияПериодДень", Тип("КнопкаФормы"), ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "ПериодДень";
	НовыйЭлемент.Заголовок 	= "День";
	//
	НовыйЭлемент 			= Элементы.Добавить("ОсновныеДействияПериодНеделя", Тип("КнопкаФормы"), ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "ПериодНеделя";
	НовыйЭлемент.Заголовок 	= "Неделя";
	//
	НовыйЭлемент 			= Элементы.Добавить("ОсновныеДействияПериодМесяц", Тип("КнопкаФормы"), ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "ПериодМесяц";
	НовыйЭлемент.Заголовок 	= "Месяц";
	//
	НовыйЭлемент 			= Элементы.Добавить("ОсновныеДействияПериодКвартал", Тип("КнопкаФормы"), ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "ПериодКвартал";
	НовыйЭлемент.Заголовок 	= "Квартал";
	//
	НовыйЭлемент 			= Элементы.Добавить("ОсновныеДействияПериодГод", Тип("КнопкаФормы"), ОсновныеДействияПодменюПериод);
	НовыйЭлемент.ИмяКоманды = "ПериодГод";
	НовыйЭлемент.Заголовок 	= "Год";
	
	НовыйЭлемент 				= Элементы.Добавить("ВариантПериода", Тип("ПолеФормы"), ОсновныеДействия);
	НовыйЭлемент.Вид			= ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным 	= "ВариантПериода";
	НовыйЭлемент.Видимость 		= Ложь;
	//
	НовыйЭлемент	 			= Элементы.Добавить("ДатаОтчета", Тип("ПолеФормы"), ОсновныеДействия);
	НовыйЭлемент.Вид			= ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным 	= "ДатаОтчета";
	НовыйЭлемент.Видимость 		= Ложь;
	Элементы.ДатаОтчета.УстановитьДействие("ПриИзменении", "ДатаОтчетаПриИзменении");
	//
	НовыйЭлемент	 						= Элементы.Добавить("ПолеОтбора", Тип("ПолеФормы"), ОсновныеДействия);
	НовыйЭлемент.Вид						= ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.ПутьКДанным 				= "ПолеОтбора";
	НовыйЭлемент.Заголовок 					= "Поле отбора";
	НовыйЭлемент.КнопкаВыпадающегоСписка 	= Истина;
	
	Элементы.ПолеОтбора.УстановитьДействие("ПриИзменении", "ПолеОтбораПриИзменении");
	Элементы.ПолеОтбора.УстановитьДействие("НачалоВыбораИзСписка", "ПолеОтбораНачалоВыбораИзСписка");

	//
	НоваяКоманда 			= Команды.Добавить("ПользовательскиеНастройкиУстановитьФлажки");
	НоваяКоманда.Действие 	= "ПользовательскиеНастройкиУстановитьФлажки";
	НоваяКоманда.Заголовок 	= "";
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.Авто;
	НоваяКоманда.ИспользуемаяТаблица = Неопределено;
	...
	Элементы.ПользовательскиеНастройкиУстановитьФлажки.ИмяКоманды = "ПользовательскиеНастройкиУстановитьФлажки";
	//
	НоваяКоманда 			= Команды.Добавить("ПользовательскиеНастройкиСнятьФлажки");
	НоваяКоманда.Действие 	= "ПользовательскиеНастройкиСнятьФлажки";
	НоваяКоманда.Заголовок 	= "";
	НоваяКоманда.ИзменяетСохраняемыеДанные = Истина;
	НоваяКоманда.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтроки.Авто;
	НоваяКоманда.ИспользуемаяТаблица = Неопределено;
	...
	НоваяКоманда.Отображение = ОтображениеКнопки.Картинка;
	НоваяКоманда.Подсказка = "";
	Элементы.ПользовательскиеНастройкиСнятьФлажки.ИмяКоманды = "ПользовательскиеНастройкиСнятьФлажки";
	
КонецПроцедуры

&НаКлиенте
Процедура СброситьНастройкиОтчета(Команда)
	...
	ЗаголовокВопроса = НСтр("ru = 'Сброс настроек'");
	ТекстВопроса = НСтр("ru = 'Сбросить настройки отчета в значение по умолчанию?'");
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Сброс'"));
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
	
	Обработчик = Новый ОписаниеОповещения("СброситьНастройкиОтчетаНаКлиенте", ЭтотОбъект);
	ПоказатьВопрос(Обработчик, ТекстВопроса, Кнопки, , КодВозвратаДиалога.Отмена, ЗаголовокВопроса);

КонецПроцедуры

&НаКлиенте
Процедура СброситьНастройкиОтчетаНаКлиенте(Результат, Параметры) Экспорт
	...
	ИдентификаторСтроки = СтрокаОтчеты.ПолучитьИдентификатор();
	СброситьНастройкиОтчетаНаСервере(ИдентификаторСтроки);
	
КонецПроцедуры

&НаСервере
Процедура СброситьНастройкиОтчетаНаСервере(ИдентификаторСтроки)
	...
	СтрокаОтчеты.АдресНастройкиВариантаОтчета = "";

	ПараметрыОтчета = ИнициализироватьОтчет(СтрокаОтчеты, Ложь, Неопределено, Неопределено);
...
Процедура ПолеОтбораНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	Если ТипЗнч(ТипПолучателейРассылки) = Тип("ОписаниеТипов") И ТипПолучателейРассылки.Типы().Количество()>0 Тогда
		...
		Для Каждого ТекЭлемент Из ЭтотОбъект.СписокВыбораПоляОтбора Цикл
...
&НаКлиенте
Процедура ПользовательскиеНастройкиУстановитьФлажки(Команда)

	ПользовательскиеНастройкиФлажки(Истина);

КонецПроцедуры

&НаКлиенте
Процедура ПользовательскиеНастройкиСнятьФлажки(Команда)

	ПользовательскиеНастройкиФлажки(Ложь);

КонецПроцедуры

&НаКлиенте
Процедура ПользовательскиеНастройкиФлажки(Использование)
	
	Для Каждого ПользовательскиеНастройкиЭлемент Из КомпоновщикНастроекКД.ПользовательскиеНастройки.Элементы Цикл
    	ПользовательскиеНастройкиЭлемент.Использование = Использование;
	КонецЦикла;
	
	ПользовательскиеНастройкиПриИзменении(Элементы.ПользовательскиеНастройкиУстановитьФлажки);
...
Процедура ПредварительныйПросмотрОтчета(Команда)
	...
	// АльфаАвто
	ПараметрыОтчета.Вставить("НастройкиВариантаОтчета", Неопределено);
	// Конец АльфаАвто
	...
	// АльфаАвто
	ПараметрыОтчета.Вставить("ВидПериода",           ЭтотОбъект.ВидПериода);
	ПараметрыОтчета.Вставить("ПолеОтбора",           ЭтотОбъект.ПолеОтбора);
	ПараметрыОтчета.Вставить("НачалоПериода",        ЭтотОбъект.ДатаОтчета.ДатаНачала);
	ПараметрыОтчета.Вставить("КонецПериода",         ЭтотОбъект.ДатаОтчета.ДатаОкончания);
	// Конец АльфаАвто
	Если СтрокаОтчеты.СКД Тогда
		ПараметрыОтчета.Настройки = КомпоновщикНастроекКД.ПользовательскиеНастройки;
		// АльфаАвто
		ПараметрыОтчета.НастройкиВариантаОтчета = КомпоновщикНастроекКД.Настройки;
		// Конец АльфаАвто
	Иначе
...
Процедура ПредварительныйПросмотрОтчетаПродолжение(РезультатВыбора, ПараметрыОтчета) Экспорт
	...
	// АльфаАвто
	НастройкиВариантаОтчета = ПараметрыОтчета.НастройкиВариантаОтчета;
	// Конец АльфаАвто
	Фильтр = Новый Структура("Использование, Значение", Истина, ШаблонЗначенияПолучателяРассылки(ПараметрыТекстаПисьмаИФайлов));
	...
	Иначе
		ПараметрыФормы.Вставить("ПользовательскиеНастройки", ПользовательскиеНастройкиКД);
			...
			ПараметрыФормы.Вставить("Отбор", Отбор);
	КонецЕсли;
	Если ПараметрыОтчета.Свойство("НастройкиВариантаОтчета") Тогда
		ПараметрыФормы.Вставить("НастройкиВариантаОтчета", ПараметрыОтчета.НастройкиВариантаОтчета);
	Иначе
		ПараметрыФормы.Вставить("НастройкиВариантаОтчета", Неопределено);
	КонецЕсли;
	// Конец АльфаАвто
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		// АльфаАвто
		Элементы.ПолеОтбора.Доступность = (Форма.ВидРассылки = "Персонализирована");
		Элементы.ПолеОтбора.Видимость = Элементы.ПолеОтбора.Доступность;
		// Конец АльфаАвто
...
&НаСервере
// АльфаАвто
Функция ОтчетыПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки, ДобавитьДежурнуюФразу = Истина, Знач ПользовательскиеНастройки = Неопределено, Знач НастройкиВариантаОтчета = Неопределено)
...
// Конец АльфаАвто
	...
	Если НастройкиВариантаОтчета = Неопределено Тогда
		...
		Иначе
			Если ОбъектСтрокаОтчеты = Неопределено Тогда
				НастройкиВариантаОтчета = Неопределено;
			Иначе
				НастройкиВариантаОтчета = ОбъектСтрокаОтчеты.НастройкиВариантаОтчета.Получить();
			КонецЕсли;
		КонецЕсли;
	...
	// Инициализация
	// АльфаАвто
	ПараметрыОтчета = ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, НастройкиВариантаОтчета);
	// Конец АльфаАвто
...
Процедура ПрочитатьНастройкиОтчетовОбъектаКопирования()
	...
	Для ОбратныйИндекс = 1 По КоличествоСтрок Цикл
		...
		// АльфаАвто
		НастройкиВариантаОтчета = СтрокаОтчетыОбъектаКопирования.НастройкиВариантаОтчета.Получить();
		// Конец АльфаАвто
		...
		ИдентификаторСтроки = СтрокаОтчеты.ПолучитьИдентификатор();
		// АльфаАвто
		СтрокаПредупреждения = ОтчетыПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки, Истина, ПользовательскиеНастройкиКД, НастройкиВариантаОтчета);
		// Конец АльфаАвто
		Если Индекс = 0 Тогда
			ЗаписатьНастройкиСтрокиОтчеты(ИдентификаторТекущейСтрокиТаблицыОтчетов);
		КонецЕсли;
...
Процедура ЗаписатьНастройкиСтрокиОтчеты(ИдентификаторСтроки)
	...
	Если Не СтрокаОтчеты.Инициализирован Тогда
		// АльфаАвто
		СохраняемоеЗначениеВариантаОтчета = Неопределено;
		// Конец АльфаАвто
		СохраняемоеЗначение = Неопределено;
	ИначеЕсли СтрокаОтчеты.СКД Тогда
		// АльфаАвто
		СтрокаОтчеты.ВидПериода = ЭтотОбъект.ВидПериода;
		СтрокаОтчеты.ПолеОтбора = ЭтотОбъект.ПолеОтбора;
		Если СтрокаОтчеты.ВидПериода = "Произвольный" Тогда
			Если ЭтотОбъект.ДатаОтчета.Вариант = ВариантСтандартногоПериода.ПроизвольныйПериод Тогда
				СтрокаОтчеты.НачалоПериода = ЭтотОбъект.ДатаОтчета.ДатаНачала;
				СтрокаОтчеты.КонецПериода  = ЭтотОбъект.ДатаОтчета.ДатаОкончания;
			Иначе
				СтрокаОтчеты.НачалоПериода = ЭтотОбъект.ДатаОтчета.Вариант;
			КонецЕсли;
		КонецЕсли;
		СохраняемоеЗначениеВариантаОтчета = КомпоновщикНастроекКД.Настройки;
		// Конец АльфаАвто
	...
	// АльфаАвто
	Адрес = ?(ЭтоАдресВременногоХранилища(СтрокаОтчеты.АдресНастройкиВариантаОтчета), СтрокаОтчеты.АдресНастройкиВариантаОтчета, УникальныйИдентификатор);
	СтрокаОтчеты.АдресНастройкиВариантаОтчета = ПоместитьВоВременноеХранилище(СохраняемоеЗначениеВариантаОтчета, Адрес);
	// Конец АльфаАвто
...
&НаСервере
// АльфаАвто
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, НастройкиВариантаОтчета = Неопределено, Интерактивно = Истина)
// Конец АльфаАвто
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru = 'Рассылка отчетов. Инициализация отчета'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	// АльфаАвто
	// Инициализация отчета
	ПараметрыОтчета = Новый Структура("Отчет, Настройки, НастройкиВариантаОтчета",
		СтрокаОтчеты.Отчет, ПользовательскиеНастройки, НастройкиВариантаОтчета);
	...
	Если СтрокаОтчеты.СКД Тогда
		...
		СтрокаОтчеты.ПолноеИмя    = ПараметрыОтчета.Метаданные.ПолноеИмя();
		
		ЭтотОбъект.ВидПериода = ПараметрыОтчета.ВидПериода;
		Если НЕ ЗначениеЗаполнено(ЭтотОбъект.ВидПериода) Тогда
			ЭтотОбъект.ВидПериода = ВариантыОтчетовАльфаАвтоКлиентСервер.ПолучитьВидПериода(СтрокаОтчеты.ПолноеИмя, СтрокаОтчеты.КлючВарианта);
		КонецЕсли;
		
		Если ТипЗнч(ПараметрыОтчета.НачалоПериода) = Тип("Дата") Тогда
			ЭтотОбъект.ДатаОтчета.ДатаНачала = ПараметрыОтчета.НачалоПериода;
			ЭтотОбъект.ДатаОтчета.ДатаОкончания = ПараметрыОтчета.КонецПериода;
		Иначе
			ВариантПериодаОтчета = Неопределено;
			Попытка
				ВариантПериодаОтчета = ВариантСтандартногоПериода[СтрЗаменить(ПараметрыОтчета.НачалоПериода, " ", "")];
			Исключение
			КонецПопытки;
			
			Если ВариантПериодаОтчета = Неопределено Тогда
				...
				ЭтотОбъект.ДатаОтчета.ДатаНачала 	= НачалоПериода.Дата;
				ЭтотОбъект.ДатаОтчета.ДатаОкончания = КонецПериода.Дата;
			Иначе
				ЭтотОбъект.ДатаОтчета = Новый СтандартныйПериод(ВариантПериодаОтчета);
			КонецЕсли;
		КонецЕсли;
		
		ЭтотОбъект.ЕстьНачалоПериода     = ПараметрыОтчета.ЕстьНачалоПериода;
		ЭтотОбъект.ЕстьКонецПериода      = ПараметрыОтчета.ЕстьКонецПериода;
		ЭтотОбъект.ПолеОтбора            = ПараметрыОтчета.ПолеОтбора;// -Рарус
		
		ОтчетыПлатформаКлиентСервер.УстановитьПометкуНаПериод(ЭтотОбъект, "Период"+ЭтотОбъект.ВидПериода);
		ВидимостьПериода();
		
		ЭтотОбъект.СписокВыбораПоляОтбора.Очистить();
		
		Для Каждого ТекЭлемент Из ПараметрыОтчета.СписокВыбораПоляОтбора Цикл
			ЭтотОбъект.СписокВыбораПоляОтбора.Добавить(ТекЭлемент.Значение, ТекЭлемент.Представление);
		КонецЦикла;
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		// АльфаАвто
		НастройкиВариантаОтчета = СтрокаОтчетыПараметры.НастройкиВариантаОтчета;
